...
export default class CartItemForDesktop extends React.Component {
    ...
    componentDidMount() {
        ...
        if (
            ...
            this.props.displayToast(this.props.product.exchangeInfo.exchangeCancelMessage);
        }
    }
    ...
    handleRemove(entryNumber, mainProductUssid, isForDigitalBundledProduct) {
        if (this.props.onRemove) {
            this.props.onRemove(entryNumber, mainProductUssid, isForDigitalBundledProduct);
    ...
    async verifyIMEINumber() {
        if (this.props) {
            ...
            let response = await this.props.verifyIMEINumber(
                ...
                this.props.product.exchangeDetails.exchangePriceDetail.exchangeAmountCashify.value,
                this.props.product.exchangeDetails.exchangePriceDetail.TULBump
                    ...
                    : null,
                this.props.product.exchangeDetails.exchangePriceDetail.pickupCharge.value,
                this.props.product.productcode,
                this.props.product.USSID,
                guid,
                this.props.product.entryNumber
            );
            if (response.status && response.status.toLowerCase() === "success" && response.isIMEIVerified) {
    ...
    removeAppliancesExchange(ussid) {
        ...
        let cartExchangeDetails = acCartExchangeDetails && JSON.parse(acCartExchangeDetails);
        if (cartExchangeDetails && cartExchangeDetails.length > 0) {
            ...
            if (index !== -1) {
                ...
                localStorage.setItem(AC_CART_EXCHANGE_DETAILS, JSON.stringify(cartExchangeDetails));
            }
    ...
    render() {
        ...
        if (this.props.product && (this.props.product.exchangeDetails || isDigitalBundledProduct)) {
        ...
        let dayFormat = "";
        let nextDayFormat = "";
        if (this.props.isFromCnc && this.props.storeDetails && this.props.storeDetails.displayName) {
            let day = new Date();
            ...
            productDayFormatOfClqAndPiq = format(pickUpDateDetails && pickUpDateDetails.pickupDate, "DD-MMM-YYYY");
            var dateForPiq = new Date(pickUpDateDetails && pickUpDateDetails.pickupDate);
        ...
        let parsedComboDiscount = this.props.product.comboDiscount && parseFloat(this.props.product.comboDiscount);
        ...
        return (
            <div className={styles.base}>
                ...
                <div className={styles.productInformation}>
                    <div className={styles.productInformationHolder}>
                        {this.props.productName && (
                            <div className={styles.informationText}>{this.props.productName}</div>
                        )}
                        <div className={styles.textWithOutOfStock}>
                            {this.props.isGiveAway === YES && <div className={styles.isGiveAwayText}>FREE</div>}
                            {this.props.isGiveAway === NO && (
                                <div className={styles.informationTextWithBolder}>
                                    ...
                                    {this.props.offerPrice && (
                                        <React.Fragment>{` ${RUPEE_SYMBOL}${this.props.offerPrice}`}</React.Fragment>
                                    )}
                                </div>
                            )}
                            {this.props.product &&
                            ...
                            {this.props.isGiveAway === NO &&
                                (!this.props.productIsServiceable && productMessage
                                    ...
                                    : (this.props.isOutOfStock || this.props.productOutOfStocks) && (
                                          <React.Fragment>
                                              <div className={styles.space}>|</div>
                                              <div className={styles.serviceAvailabilityText}>
                                                  {/* OUT_OF_STOCK */}
                        ...
                        {this.props.product.comboDiscount && parsedComboDiscount && parsedComboDiscount !== 0 && (
                            <ComboOfferSection
                                ...
                                comboDiscountAppliedQuantity={this.props.product.comboDiscountAppliedQuantity}
                        ...
                        {this.props.isGiveAway === YES && (
                            <div className={styles.isGiveAwayQuantity}>
                                Quantity:
                                <span className={styles.isGiveAwayQuantityValue}>{this.props.qtySelectedByUser}</span>
                            </div>
                        )}
                        ...
                        {(this.props.size || this.props.color) && (
                            <div className={styles.colourSizeHolder}>
                                {this.props.color && (
                                    <div className={styles.sizeText}>{`Color:  ${this.props.color}`}</div>
                                )}
                                {this.props.size && this.props.size.toUpperCase() !== NO_SIZE && (
                                    <div className={styles.colourText}>{`${SizeType}: ${sizeValue}`}</div>
                                )}
                            </div>
                        )}
                    </div>
                        ...
                        <div className={styles.footer}>
                            <div className={styles.wishlist}>
                                <AddToWishListButtonContainer
                                    ...
                                    entryNumber={this.props.entryNumber}
                                    isFromCartPage={true}
                                    removeAppliancesExchange={ussid => this.removeAppliancesExchange(ussid)}
                                />
                            </div>
                            <div
                                className={styles.removeLabel}
                                onClick={() =>
                                    this.handleRemove(this.props.product.entryNumber, this.props.product.USSID)
                                }
                            >
                ...
                {!this.props.isFromCnc && this.props.isGiveAway === NO && this.props.deliveryInformation && (
                    <div className={styles.deliveryInfo}>
                        <DeliveryInfoSelect
                            ...
                            deliveryInformationWithDate={this.props.deliveryInformationWithDate}
                ...
                {this.props.product.exchangeDetails && this.props.product.exchangeDetails.exchangeModelName && (
                    <React.Fragment>
                        <div
                            className={
                        ...
                        >
                            <div className={styles.exchangeDetailsSectionOne}>
                                ...
                                <div className={styles.exchangeDetailsHeading}>
                                    ...
                                    {this.props.product.exchangeDetails.quoteExpired && <span> has been updated</span>}
                                </div>
                            </div>
                            <div className={styles.exchangeDetailsSectionTwo}>
                                {this.props.product.exchangeDetails.quoteExpired ? (
                                    <div className={styles.exchangePriceNDetails}>
                                        <div className={styles.getNewPrice} onClick={() => this.verifyIMEINumber()}>
                                            Get New Price
                                        </div>
                                    </div>
                                ) : (
                            ...
                            {this.state.showMore && (
                                <React.Fragment>
                                    ...
                                    {this.props.product.exchangeDetails.exchangePriceDetail &&
                                        this.props.product.exchangeDetails.exchangePriceDetail.TULBump && (
                                            <React.Fragment>
                                                <div className={styles.font14LightLeft}>CLiQ Exclusive Cashback</div>
                                                <div className={styles.font14LightRight}>
                                                    {
                                                        this.props.product.exchangeDetails.exchangePriceDetail.TULBump
                                    ...
                                    <div className={styles.font12light}>
                                        ...
                                        <span className={styles.tncLink} onClick={() => this.openTnCModal()}>
                                            T&amp;C
                                        </span>{" "}
                                    </div>
                                    <div
                ...
                <AppliancesExchangeCart
                    ...
                    appliancesExchangePincodeData={this.props.appliancesExchangePincodeData}
                    productCode={this.props.product.productcode}
                />

                {this.props.product.bundledDigitalItems &&
                    this.props.product.bundledDigitalItems.map((digitalProduct, index) => {
                        return (
                            <DigitalBundledProduct
                                key={index}
                                digitalProduct={digitalProduct}
                                mainProductUssid={this.props.product.USSID}
                                onRemove={(entryNumber, mainProductUssid, isForDigitalBundledProduct) =>
                                    this.handleRemove(entryNumber, mainProductUssid, isForDigitalBundledProduct)
                                }
                ...
                {this.props.product.bundlingSuggestionAvailable &&
                    ...
                    this.props.productIsServiceable && (
                        <RecommendedBundledProduct
                            ...
                            bundledProductSuggestionDetails={this.props.bundledProductSuggestionDetails}
                            ...
                            displayToast={this.props.displayToast}
                            history={this.props.history}
                            bundledProductSuggestionStatus={this.props.bundledProductSuggestionStatus}
                        />
                    )}
...
CartItemForDesktop.propTypes = {
    ...
    verifyIMEINumber: PropTypes.string,
    getCartDetails: PropTypes.func,
    productNotServiceable: PropTypes.string,
    productOutOfStocks: PropTypes.string,
    storeDetails: PropTypes.object,
    deliveryInformationWithDate: PropTypes.arrry,
    isFromCnc: PropTypes.bool,
    sizeType: PropTypes.string,
    size: PropTypes.string,
    isGiveAway: PropTypes.string,
    offerPrice: PropTypes.string,
    discount: PropTypes.string,
    productIsServiceable: PropTypes.string,
    ...
    allStores: PropTypes.object,
    isTop: PropTypes.bool,
    inCartPage: PropTypes.string,
