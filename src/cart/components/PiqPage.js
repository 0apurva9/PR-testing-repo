...
const WestSideIcon = "https://assets.tatacliq.com/medias/sys_master/pwaImg/google_search.png";
...
export default class PiqPage extends React.Component {
    constructor(props) {
        ...
        this.state = {
            lat:
                this.props.availableStores && this.props.availableStores.length > 0 && this.props.availableStores[0]
                    ? this.props.availableStores && this.props.availableStores[0].geoPoint.latitude
                    : 28.6129918,

            lng:
                this.props.availableStores && this.props.availableStores.length > 0 && this.props.availableStores[0]
                    ? this.props.availableStores && this.props.availableStores[0].geoPoint.longitude
                    : 77.2310456,
    ...
    handleSwipe(val) {
        const lat = this.props.availableStores[val % this.props.numberOfStores].geoPoint.latitude;
        const lng = this.props.availableStores[val % this.props.numberOfStores].geoPoint.longitude;
        ...
        return (
            <div className={styles.base}>
                ...
                <div className={styles.bannerMobileHolder}>
                    {this.props.availableStores && this.props.availableStores.length > 1 && (
                        <React.Fragment>
                            {!this.props.showPickupPerson && this.props.availableStores && (
                                <BannerMobile onSwipe={val => this.handleSwipe(val)} bannerHeight="55vw">
                                    {this.props.availableStores.map((val, i) => {
                                        return (
                                            <PickUpLocation
                                                key={i}
                        ...
                        <div className={styles.getLocationDetailsHolder}>
                            <div className={styles.getLocationDetails}>
                                <GetLocationDetails
                                    ...
                                    address={`${selectedStore.returnAddress1} ${selectedStore.returnAddress2} ${selectedStore.returnCity}`}
                                    pickUpKey="Open on: "
