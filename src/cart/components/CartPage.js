...
class CartPage extends React.Component {
    ...
    displayCoupons = () => {
        ...
        if (userDetails !== undefined && customerCookie !== undefined && cartDetailsLoggedInUser !== undefined) {
    ...
    componentDidMount() {
        ...
        if (userDetails !== undefined && customerCookie !== undefined && cartDetailsLoggedInUser !== undefined) {
            ...
            if (this.props.location && this.props.location.state && this.props.location.state.isFromCliqAndPiq) {
                cliqPiqCartCode =
                    this.props.location && this.props.location.state && this.props.location.state.isCliqAndPiqCartCode
            ...
            this.props.getCartDetails(
                JSON.parse(userDetails).userName,
                JSON.parse(customerCookie).access_token,
                cliqPiqCartCode ? cliqPiqCartCode : JSON.parse(cartDetailsLoggedInUser).code,
                defaultPinCode
            );
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.location && nextProps.location.state && nextProps.location.state.isFromCliqAndPiq) {
            this.setState({ isComingFromCliqAndPiq: true });
        }
        if (this.props.cart.coupons !== nextProps.cart.coupons) {
            let couponDetails = nextProps.cart && Object.assign(nextProps.cart.coupons, nextProps);
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.cart) {
            if (this.props.location.state && this.props.location.state.isClickOnAddToBag) {
                window.scrollTo(0, document.body.scrollHeight - window.outerHeight - 100);
            }
            if (prevProps.cart.cartDetails !== this.props.cart.cartDetails) {
                ...
                let productServiceAvailability = filter(
                    this.props.cart && this.props.cart.cartDetails && this.props.cart.cartDetails.products,
                    product => {
                        return (
    ...
    removeItemFromCart = (entryNumber, mainProductUssid, isForDigitalBundledProduct) => {
        const pinCode = localStorage.getItem(DEFAULT_PIN_CODE_LOCAL_STORAGE);
    ...
    renderToCheckOutPage(productExchangeServiceable) {
        ...
        if (productExchangeServiceable && productExchangeServiceable.includes(true)) {
    ...
    checkPinCodeAvailability = (val, addressId = "") => {
        ...
        if (storeDetails && localStorage.getItem(DEFAULT_PIN_CODE_LOCAL_STORAGE) !== val) {
            localStorage.removeItem(SELECTED_STORE);
        }
    ...
    changePinCode = () => {
        ...
        this.props.addressModal({
            ...
            labelText: "Submit",
            checkPinCodeAvailability: (pinCode, addressId) => this.checkPinCodeAvailability(pinCode, addressId),
        });
    };
            ...
            return (
                <div className={styles.card}>
                    <div className={styles.content}>
                        ...
                        {this.props.cart.paymentModes.paymentOffers.coupons.map((val, i) => {
                            return (
                                <div className={styles.row} key={i}>
                                    <div className={styles.bankOfferHeading}>{val.offerTitle}</div>
                                    <div className={styles.bankOfferText}>{val.offerDescription}</div>
                                </div>
                            );
                        })}
                    </div>
                ...
                <DesktopOnly>
                    <div className={styles.changePinCodeHolder}>
                        <div className={styles.checkHolder}>
                            ...
                            <div className={!defaultPinCode ? styles.inputAndButtonHolder : styles.forDefault}>
                                <div className={styles.inputAndButton}>
                                    <TextWithUnderLine
                                        onClick={() => this.changePinCode()}
                                        buttonLabel="Change PIN code"
                                        checkPinCodeAvailability={pinCode => this.checkPinCodeAvailability(pinCode)}
                ...
                <MobileOnly>
                    <div className={styles.content}>
                        <TextWithUnderLine
                            ...
                            boxShadow={defaultPinCode && defaultPinCode !== "undefined" ? true : false}
                            onClick={() => this.changePinCode()}
                            buttonLabel="Change"
                        />
                    </div>
    ...
    getAllStores = async selectedProductsUssId => {
        ...
        let getAllStoresCNCResponse = await this.props.getAllStoresCNC(defalutPinCode);
    ...
    validateLocalStorageProducts() {
        let cartProducts = this.props.cart && this.props.cart.cartDetails && this.props.cart.cartDetails.products;
    ...
    render() {
        ...
        if (this.props.cart.loadingForDisplayCoupon || this.props.cart.loadingForCartDetail) {
                    ...
                    <div className={styles.pageCenter}>
                        <DesktopOnly>
                            <div className={styles.offerText}>
                                Apply a relevant <span>coupon code</span> here to avail any additional discount.
                                Applicable <span>cashback</span> if any will be credited to your account as per T&C.
                            </div>
                        </DesktopOnly>
                                    ...
                                    cartDetails.products.map((product, i) => {
                                        ...
                                        return (
                                            <div className={styles.cartItem} key={i}>
                                                <MobileOnly>
                                                    <CartItem
                                                        ...
                                                        productNotServiceable={
                                                            product.pinCodeResponse &&
                                                            product.pinCodeResponse.productNotServiceabilityMessage
                                                        }
                                                ...
                                                <DesktopOnly>
                                                    <CartItemForDesktop
                                                        isCod={product.pinCodeResponse && product.pinCodeResponse.cod}
                                                        isFromCnc={
                                                            (this.props.location &&
                                                        ...
                                                        isClickable={false}
                                                        onClickImage={() => this.onClickImage(product.productcode)}
                                                        showExchangeTnCModal={this.props.showExchangeTnCModal}
                                                        ...
                                                        isShippingObjAvailable={
                                                            ...
                                                            this.props.cart.cartDetails.cartAmount.shippingCharge
                                                                ? true
                                                                : false
                                                        }
                                        ...
                                        <Checkout
                                            ...
                                            amount={
                                                cartDetails.cartAmount.paybleAmount.value
                                                    ? Math.round(cartDetails.cartAmount.paybleAmount.value * 100) / 100
                                                    : "0.00"
                                            }
                                            bagTotal={
                                                cartDetails.cartAmount.bagTotal.value
                                                    ? Math.round(cartDetails.cartAmount.bagTotal.value * 100) / 100
                                                    : "0.00"
                                            }
                                    ...
                                    {this.state.showCheckoutSection && cartDetails.products && cartDetails.cartAmount && (
                                        <div className={styles.amountDetails}>
                                            <DesktopCheckout
                                                ...
                                                amount={
                                                    cartDetails.cartAmount.paybleAmount.value
                                                        ? Math.round(cartDetails.cartAmount.paybleAmount.value * 100) /
                                                          100
                                                        : "0.00"
                                                }
                                                carPageBagTotal={
                                                ...
                                                onCheckout={() => this.renderToCheckOutPage(productExchangeServiceable)}
                                                label={CHECKOUT__TEXT}
                                                isOnCartPage={true}
                                                changePinCode={this.changePinCode}
                                                isFromMyBag={true}
                                                showDetails={this.state.showCartDetails}
                                                productExchangeServiceable={productExchangeServiceable}
                                                totalExchangeAmount={cartDetails.totalExchangeAmount}
                                                isQuoteExpired={isQuoteExpired}
                                                isShippingObjAvailable={
                                                    cartDetails.cartAmount && cartDetails.cartAmount.shippingCharge
                                                        ? true
                                                        : false
                                                }
                                                shippingPromoMessage={cartDetails.shippingPromoMessage}
                                                showShippingMsg={true}
                                                appliancesExchangePincodeData={this.state.appliancesExchangePincodeData}
                                                cartProducts={cartDetails.products}
                                            />
                                        </div>
                                    )}
                                    {this.props && this.props.wishListCount !== null && this.props.wishListCount > 0 && (
