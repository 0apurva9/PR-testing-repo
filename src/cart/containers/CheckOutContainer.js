...
import { setDataLayerForCheckoutDirectCalls, ADOBE_CALL_FOR_SEE_ALL_BANK_OFFER } from "../../lib/adobeUtils";
import { setUrlToRedirectToAfterAuth } from "../../auth/actions/auth.actions.js";
import { SUCCESS_CAMEL_CASE, SUCCESS_UPPERCASE, SUCCESS, MY_ACCOUNT } from "../../lib/constants.js";
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        getCartDetailsCNC: (userId, accessToken, cartId, pinCode, isSoftReservation) => {
            dispatch(
                getCartDetailsCNC(
                    userId,
                    accessToken,
        ...
        getCODEligibility: (isPaymentFailed, isFromRetryUrl, retryCartGuid) => {
            dispatch(getCODEligibility(isPaymentFailed, isFromRetryUrl, retryCartGuid));
        },
        ...
        updateTransactionDetailsForCOD: (paymentMode, juspayOrderID, isFromRetryUrl, retryCartGuid) => {
            dispatch(updateTransactionDetailsForCOD(paymentMode, juspayOrderID, isFromRetryUrl, retryCartGuid));
        },
        ...
        binValidationForNetBanking: (paymentMode, binNo, isFromRetryUrl, retryCartGuid) => {
            dispatch(binValidationForNetBanking(paymentMode, binNo, isFromRetryUrl, retryCartGuid));
        },
        softReservationPaymentForNetBanking: (paymentMethodType, paymentMode, bankCode, pinCode, bankName) => {
            dispatch(softReservationPaymentForNetBanking(paymentMethodType, paymentMode, bankCode, pinCode, bankName));
        },
        softReservationPaymentForSavedCard: (cardDetails, address, paymentMode, isPaymentFailed) => {
            dispatch(softReservationPaymentForSavedCard(cardDetails, paymentMode, isPaymentFailed));
        },
        ...
        createJusPayOrderForGiftCardNetBanking: (guId, bankCode, bankName) => {
            dispatch(createJusPayOrderForGiftCardNetBanking(guId, bankCode, bankName));
        },
        ...
        getBankAndTenureDetails: (retryFlagForEmiCoupon, isFromRetryUrl, retryCartGuid, isFromDebitCard) => {
            dispatch(getBankAndTenureDetails(retryFlagForEmiCoupon, isFromRetryUrl, retryCartGuid, isFromDebitCard));
        },
        ...
        getItemBreakUpDetails: (couponCode, cartGuId, noCostEmiText, noCostProductCount, emiInfo) => {
            dispatch(getItemBreakUpDetails(couponCode, cartGuId, noCostEmiText, noCostProductCount, emiInfo));
        },
        getPinCode: pinCode => {
        ...
        createJusPayOrderForSavedCards: (cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid) => {
            dispatch(
                createJusPayOrderForSavedCards(cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid)
            );
        },
        createJusPayOrderForCliqCash: (pinCode, cartItem, isPaymentFailed) => {
            dispatch(createJusPayOrderForCliqCash(pinCode, cartItem, isPaymentFailed));
        },
        clearCartDetails: () => {
        ...
        getUserAddressAndDeliveryModesByRetryPayment: async guId => {
            ...
            if (
                ...
                getUserAddressAndDeliveryModesResponse.getUserAddressAndDeliveryModesByRetryPayment.products &&
                getUserAddressAndDeliveryModesResponse.status === SUCCESS
            ) {
                let selectedDeliveryMode =
                    getUserAddressAndDeliveryModesResponse.getUserAddressAndDeliveryModesByRetryPayment.products[0] &&
                    getUserAddressAndDeliveryModesResponse.getUserAddressAndDeliveryModesByRetryPayment.products[0]
                        .selectedDeliveryModeCode;
                let validDeliveryModes =
                    getUserAddressAndDeliveryModesResponse.getUserAddressAndDeliveryModesByRetryPayment
                        .pinCodeResponseList[0].validDeliveryModes &&
                ...
                if (
                    getUserAddressAndDeliveryModesResponse.getUserAddressAndDeliveryModesByRetryPayment
                        .pinCodeResponseList[0].isServicable === "N"
                ) {
                    dispatch(
                        displayToast(
                    ...
                    dispatch(displayToast("We're sorry, we don't service this PIN code right now"));
                    ownProps.history.push(MY_ACCOUNT);
                }
            }
        },
        retryPayment: async (retryPaymentGuId, retryPaymentUserId) => {
            let retryPaymentResponse = await dispatch(retryPayment(retryPaymentGuId, retryPaymentUserId));
        ...
        stripe_juspay_TokenizeGiftCard: (cardDetails, paymentMode, egvCartGuid) => {
            dispatch(stripe_juspay_TokenizeGiftCard(cardDetails, paymentMode, egvCartGuid));
        },
        ...
        stripeTokenize: (cardDetails, address, cartItem, paymentMode, isPaymentFailed) => {
            dispatch(stripeTokenize(cardDetails, address, cartItem, paymentMode, isPaymentFailed));
        },
        getMinicartProducts: () => {
            dispatch(getMinicartProducts());
        },
        collectPaymentOrderForGiftCardNetBanking: (guId, bankCode, bankName) => {
            dispatch(collectPaymentOrderForGiftCardNetBanking(guId, bankCode, bankName));
        },
        ...
        collectPaymentOrderForSavedCards: (cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid) => {
            dispatch(
                collectPaymentOrderForSavedCards(cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid)
            );
        },
        ...
        collectPaymentOrderForCliqCash: (pinCode, cartItem, isPaymentFailed) => {
            dispatch(collectPaymentOrderForCliqCash(pinCode, cartItem, isPaymentFailed));
        },
        ...
        binValidationForUPI: async (paymentMode, isFromRetryUrl, retryCartGuid) => {
            return await dispatch(binValidationForUPI(paymentMode, isFromRetryUrl, retryCartGuid));
        },
        collectPaymentOrderForUPI: (
            paymentMethodType,
            cartItem,
        ...
        createJusPayOrderForUPI: (cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid) => {
            dispatch(createJusPayOrderForUPI(cardDetails, cartItem, isPaymentFailed, isFromRetryUrl, retryCartGuid));
        },
        upiPaymentIsNewMidddleLayer: () => {
        ...
        softReservationPaymentForUPI: (paymentMethodType, paymentMode, bankCode, pinCode, bankName) => {
            dispatch(softReservationPaymentForUPI(paymentMethodType, paymentMode, bankCode, pinCode, bankName));
        },
...
const CheckoutAddressContainer = withRouter(connect(mapStateToProps, mapDispatchToProps)(CheckOutPage));
