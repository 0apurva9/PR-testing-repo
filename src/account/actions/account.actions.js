...
export const TICKET_RECENT_HISTORY_DETAILS_REQUEST = "TICKET_RECENT_HISTORY_DETAILS_REQUEST";
export const TICKET_RECENT_HISTORY_DETAILS_SUCCESS = "TICKET_RECENT_HISTORY_DETAILS_SUCCESS";
export const TICKET_RECENT_HISTORY_DETAILS_FAILURE = "TICKET_RECENT_HISTORY_DETAILS_FAILURE";

export const RESET_TICKETS_HISTORY_DATA_TO_INITIAL = "RESET_TICKETS_HISTORY_DATA_TO_INITIAL";
...
export const SUBMIT_CAPTURE_ATTACHMENTS_REQUEST = "SUBMIT_CAPTURE_ATTACHMENTS_REQUEST";
export const SUBMIT_CAPTURE_ATTACHMENTS_SUCCESS = "SUBMIT_CAPTURE_ATTACHMENTS_SUCCESS";
export const SUBMIT_CAPTURE_ATTACHMENTS_FAILURE = "SUBMIT_CAPTURE_ATTACHMENTS_FAILURE";
...
export function addUserUPIRequest() {
    return {
...
export function getAllOrdersSuccess(orderDetails, isPaginated) {
    return {
...
export function getAllOrdersDetails(suffix, paginated, isSetDataLayer, showDataAccordingToUser) {
    return async (dispatch, getState, { api }) => {
        const userDetails = Cookie.getCookie(LOGGED_IN_USER_DETAILS);
        const customerCookie = Cookie.getCookie(CUSTOMER_ACCESS_TOKEN);
...
export function getAllSellersDetails(isSetDataLayer) {
    true;
...
export function followAndUnFollowBrand(brandId, followStatus, pageType, positionInFeed) {
        ...
        try {
            ...
            if (!followInFeedBackApiResultJsonStatus.status) {
                ...
                if (pageType === HOME_FEED_FOLLOW_AND_UN_FOLLOW) {
                    ...
                    return dispatch(
                        followAndUnFollowBrandSuccessForHomeFeed(brandId, updatedFollowedStatus, positionInFeed)
                    );
...
const getCheckboxApiData = (apiData = []) => {
    ...
    let returnValue = {
        ...
        btnText: items && items.btnText ? items.btnText : "",
        heading:
            items && items.description ? (items.description.split("|") ? items.description.split("|")[0] : "") : "",
        isMandatory:
            items && items.description
                ? items.description.split("|")
                    ? parseInt(items.description.split("|")[1])
...
export function getOrdersTransactionDataSuccess(ordersTransactionData, isPaginated) {
    return {
...
export function getCliqCashPageConfiguration() {
    return async (dispatch, getState, { api }) => {
        const customerCookie = Cookie.getCookie(CUSTOMER_ACCESS_TOKEN);
        let userDetails = Cookie.getCookie(LOGGED_IN_USER_DETAILS);
...
export function captureAttachmentsSubmitFailure() {
    return {
...
export function getRecentTicketHistoryDetailsSuccess(ticketDetails, isPaginated = false) {
    return {
...
export function captureAttachmentsSubmit(custoemrId, sendData) {
    return async (dispatch, getState, { api }) => {
        ...
        try {
            const result = await api.post(
                `v2/mpl/docs/upload?access_token=${JSON.parse(customerCookie).access_token}&cId=${custoemrId}`,
                sendData
            );
            let resultJson = await result.json();
            if (resultJson.status === FAILURE) {
...
export function getRecentTicketHistoryDetails(paginated = false, ticketStatus = "", ticketYear = "") {
    return async (dispatch, getState, { api }) => {
        ...
        try {
            ...
            dispatch(getRecentTicketHistoryDetailsRequestFailure(e.message, paginated));
        }
    };
}
