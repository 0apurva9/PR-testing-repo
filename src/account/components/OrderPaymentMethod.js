...
import { setDataLayer, ADOBE_HELP_SUPPORT_LINK_CLICKED } from "../../lib/adobeUtils";
        ...
        return (
            <div className={styles.base}>
                {(this.props.paymentMethod || this.props.isInvoiceAvailable) && (
                    <div
                        className={this.props.clickcollect != true ? styles.marginAddress : styles.removeMarginAddress}
                    >
                            ...
                            {this.props.paymentMethod && (
                                <div className={styles.paymentMethod}>
                                    ...
                                    <span className={styles.paymentMethodMode}>
                                        {this.props.paymentMethod}{" "}
                                        {!this.props.paymentMethod.toLowerCase().includes("cliq cash") &&
                                            this.props.cliqCashAmountDeducted > 0 &&
                                            ` | CLiQ Cash`}
                                    </span>
                                </div>
                            )}
                        </div>
                        <div className={styles.helpSupport} onClick={() => this.redirectToHelpPage()}>
                            Help & Support
                        </div>
                    </div>
                )}
                {this.props.deliveryAddress && this.props.clickcollect != true && (
                    <React.Fragment>
                        <React.Fragment>
                            ...
                            {this.props.isCDA ? (
                                <div className={styles.helpSupport} onClick={() => this.onChangeAddress()}>
                                    Change
                                </div>
                            ) : (
                                ""
                            ...
                            <div className={styles.deliveryAddress}>
                                <React.Fragment>
                                    {addressLine1.addressLine1 && (
                                        <span className={styles.addressLine}>{addressLine1.addressLine1}, </span>
                                    )}
                    ...
                    <span className={styles.addressLine}>
                      {this.state.deliveryAddress.landmark},{" "}
                                    ...
                                    {addressLine1.town && (
                                        <span className={styles.addressLine}>{addressLine1.town}, </span>
                                    )}
                                    {addressLine1.state && (
                                        <span className={styles.addressLine}>{addressLine1.state}, </span>
                                    )}
                                    {addressLine1.postalcode && (
                                        <span className={styles.addressLine}>{addressLine1.postalcode}</span>
                                    )}
                                </React.Fragment>
                            </div>
                        )}
