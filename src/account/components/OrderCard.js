...
import { setDataLayer, ADOBE_ITEM_DETAILS_LINK_CLICKED } from "../../lib/adobeUtils";
...
export default class OrderCard extends React.Component {
    ...
    onViewItemDetails(orderId, transactionId) {
        ...
        this.props.history.push(`${MY_ACCOUNT}${ORDER}/?${ORDER_CODE}=${orderId}&transactionId=${transactionId}`);
    }
    ...
    showToastMessage() {
        this.props.displayToast("Your order is not confirmed yet. Please see order details.");
    }

    copyToClipBoard = event => {
        event.preventDefault();
        event.stopPropagation();
        const copyContent = this.copyText.innerHTML;
        ...
        switch (date) {
            ...
            case 31:
                if (newExpressOrSddText) {
                    return newExpressOrSddText + date + "st " + monthNames[month - 1] + " " + year;
                } else {
            ...
            case 22:
                if (newExpressOrSddText) {
                    return newExpressOrSddText + date + "nd " + monthNames[month - 1] + " " + year;
                } else {
            ...
            case 23:
                if (newExpressOrSddText) {
                    return newExpressOrSddText + date + "rd " + monthNames[month - 1] + " " + year;
                } else {
                    return "" + date + "rd " + monthNames[month - 1];
                }
            default:
                if (newExpressOrSddText) {
                    return newExpressOrSddText + date + "th " + monthNames[month - 1] + " " + year;
                } else {
    ...
    render() {
        ...
        let updatedCalloutMessage = calloutMessage && calloutMessage.replace("Date", "Date: ");
        let statusDisplayMsg = this.props.statusDisplayMsg && this.props.statusDisplayMsg;
        let estimatedDeliveryDateFormatted = "";
        let deliveryDate = "",
            deliveryDateFormatted = "";
        if (this.props.estimatedDeliveryDate && !this.props.estimatedDeliveryDate.includes("DeliveryTAT not found")) {
            estimatedDeliveryDateFormatted = this.getDayNumberSuffix(this.props.estimatedDeliveryDate);
        }
        ...
        if (statusDisplayMsg && statusDisplayMsg.length > 0) {
            ...
            let statusList = statusDisplayMsgL1 && statusDisplayMsgL1.value && statusDisplayMsgL1.value.statusList;
            let LaststatusDisplayList = statusList ? statusList[statusList.length - 1] : "";
        ...
        if (date && this.props.returnPolicy) {
            ...
            returnEligibleDate.setDate(returnEligibleDate.getDate() + parseInt(this.props.returnPolicy));
        }
        let orderCouldbeCollected = "";
        if (date && this.props.returnPolicy) {
            orderCouldbeCollected = new Date(date);
            orderCouldbeCollected.setDate(orderCouldbeCollected.getDate() + parseInt(this.props.returnPolicy));
        }
        ...
        if (this.props.clickAndCollect === true) {
            checkStatus = shipmentStatus && shipmentStatus.includes("Estimated Delivery Date");
            CNCcallOut =
                this.props &&
        ...
        return (
            <div className={this.props.onHollow ? styles.onHollow : styles.base}>
                {this.props.returnFlow && <div className={styles.cancelTitle}>Return Item</div>}
                {this.props.title && this.props.title === "Cancel Item" && !this.props.returnFlow && (
                ...
                <div
                    ...
                    style={{
                        width: this.props.imageHolderWidth ? this.props.imageHolderWidth : null,
                        height: this.props.imageHolderHeight ? this.props.imageHolderHeight : null,
                    }}
                >
                    <ProductImage
                ...
                <div className={styles.productDetails}>
                    <div className={this.props.isSelect ? styles.withCheckBox : styles.productName}>
                        {this.props.isSelect && (
                        ...
                        {this.props.isEgvOrder && (
                            <React.Fragment>
                                <div className={styles.egvCardNumber}>{this.props.egvCardNumber}</div>
                                <div className={styles.deliveryDate}>
                                    <div className={styles.egvText}>{this.props.giftCardStatus}</div>
                                </div>
                                ...
                                <div className={styles.egvAmount}>{this.props.totalFinalPayableOrderAmount}</div>
                            </React.Fragment>
                        )}
                    </div>

                    {this.props.exchangeDetails && this.props.idFromAllOrderDetails === "Y" && (
                        <div className={styles.exchangeDetailsContainer}>
                            <img src={exchangeIconLight} alt="exchange icon" className={styles.exchangeIconLight} />
                            <div className={styles.exchangeProductText}>
                                Exchange Product: {this.props.exchangeDetails.exchangeModelName}
                            </div>
                        </div>
                    )}
                    ...
                    {this.props.retryPaymentUrl && (
                        <React.Fragment>
                            <div className={styles.deliveryDate}>{this.props.retryPayment}</div>
                        </React.Fragment>
                    )}
                    {!this.props.isEgvOrder &&
                        ...
                        this.props.price != 0.01 && (
                            <div className={styles.deliveryDate}>
                                ...
                                {this.props.orderCancelDate && <span> on {this.props.orderCancelDate}</span>}
                            </div>
                        )}
                    {this.props.orderStatusCode &&
                        this.props.orderStatusCode === "DELIVERED" &&
                        this.props.deliveryDate && (
                            <div className={styles.deliveryDate}>Delivered on {this.props.deliveryDate}</div>
                        )}
                    ...
                    {this.props &&
                        ...
                        !this.props.calloutMessage.includes(EDD_TEXT) && (
                            ...
                            >
                                <div className={styles.calloutMessage}>{`${updatedCalloutMessage}`}</div>
                            </div>
                        )}
                                    ...
                                    {this.props.showIsGiveAway && (
                                        <div className={styles.price}>
                                            ...
                                            this.props.productName === "Gift Card"
                                                ...
                                                : this.props.price
                                                ? `${RUPEE_SYMBOL} ${NumberFormatter.convertNumber(this.props.price)}`
                                                : null}
                                        </div>
                                    )}
                    ...
                    {this.props.idFromAllOrderDetails != "Y" && (
                        <div className={styles.priceHolder}>
                            {this.props.productSize && (
                                <div className={styles.price}>
                                    {this.props.productSize} {this.props.productColourName ? "|" : ""}
                                </div>
                            )}
                            {this.props.productColourName && (
                                <div className={styles.price}>&nbsp;{this.props.productColourName}</div>
                            )}
                        </div>
                    )}
                    {this.props.idFromAllOrderDetails != "Y" && this.props.quantity && (
                        <div className={styles.priceWithQuantity}>
                            ...
                            <div className={styles.quantity}>{this.props.numberOfQuantity}</div>
                        </div>
                    )}
                    ...
                    {this.props.children &&
                        ...
                        this.props.productName != "Gift Card" && (
                            <div className={styles.additionalContent}>{this.props.children}</div>
                        )}
                    {this.props.orderStatusCode &&
                        this.props.orderStatusCode !== "DELIVERED" &&
                        this.props.orderStatusCode !== "PAYMENT_PENDING" &&
                            ...
                            <React.Fragment>
                                <div className={styles.edd}>
                                    <span className={styles.ffsemibold}>
                                        {this.props.clickAndCollect === true ? "Pickup Date" : EDD_TEXT}:
                                    </span>
                                    <span>
                                        &nbsp;
                                        {estimatedDeliveryDateFormatted}
                                ...
                                {this.props.orderBreachMessage && (
                                    <div className={styles.breachMessage}>* {this.props.orderBreachMessage}</div>
                                )}
                            </React.Fragment>
                        )}
                    ...
                    {!isPaymentFailure &&
                        ...
                        this.props.showRightArrow && (
                            <span
                                ...
                                onClick={() => this.onViewItemDetails(this.props.orderId, this.props.transactionId)}
                            />
                        )}
                    ...
                    {!this.props.isEgvOrder &&
                        ...
                        this.props.showRightArrow && (
                            <span className={styles.rightArrow} onClick={() => this.showToastMessage()} />
                        )}

                    {!isPaymentFailure && this.props.isGiveAway === NO && !this.props.retryPaymentUrl && (
                        <div
                            className={styles.trackOrderText}
                            onClick={() => this.onViewItemDetails(this.props.orderId, this.props.transactionId)}
                        >
                            {this.getTrackOrderText(this.props.orderStatusCode, this.props.isEgvOrder)}
                        </div>
                    )}
                            ...
                            <div className={styles.pickupAddressHolder}>
                                ...
                                {this.props.pickupAddress && (
                                    <div className={styles.pickupAddressText}>
                                        {this.props.pickupAddress.line1} {this.props.pickupAddress.line1 ? "," : ""}
                                        &nbsp;
                                        {this.props.pickupAddress.landmark}{" "}
                                        {this.props.pickupAddress.landmark ? "," : ""}&nbsp;
                                        {this.props.pickupAddress.city} {this.props.pickupAddress.city ? "," : ""}&nbsp;
                                        {this.props.pickupAddress.state} {this.props.pickupAddress.state ? "," : ""}
                                        &nbsp;
                ...
                {this.props.returnReason && (
                    <div className={styles.returnReasonText}>
                        <span className={styles.returnReasonTitle}>Reason for return:</span> {this.props.returnReason}
                    </div>
                )}
                {this.props.returnSubReason && (
                    <div className={styles.returnCommentText}>
                        <span className={styles.returnCommentTitle}>Issue Detail:</span> {this.props.returnSubReason}
                    </div>
                )}
                {this.props.returnComments && (
                    <div className={styles.returnCommentText}>
                        <span className={styles.returnCommentTitle}>Comments:</span> {this.props.returnComments}
                    </div>
                )}

                {this.props.additionalContent && <React.Fragment>{this.props.additionalContent}</React.Fragment>}
                ...
                {this.props.title === PRODUCT_CANCEL && this.props.discountBundlingCancelable && (
                    <React.Fragment>
                        <div className={styles.divider} />
                        <div className={styles.cancelExchangeMessage}>{CANCEL_REASON_FOR_BUNDLED_PRODUCT}</div>
                    </React.Fragment>
                )}

                {(this.props.title === PRODUCT_CANCEL || this.props.returnFlow) && this.props.bundledAssociatedItems && (
                    <React.Fragment>
                        {this.props.title === PRODUCT_CANCEL && !this.props.returnFlow && (
                        ...
                        {this.props.bundledAssociatedItems.map((bundledProduct, index) => {
                            return (
                ...
                {this.props.selectedDeliveryMode &&
                    ...
                    this.props.selectedDeliveryMode.name.toLowerCase() === "click and collect" && (
                        <div className={styles.commonTitle}>
                            <span className={styles.ffsemibold}>Delivery Mode: </span>
                            <span className={styles.estimatedDate}>
                                {this.getSelectedDeliveryModeName(this.props.selectedDeliveryMode.name)}
                            </span>
                        </div>
                    )}
                {this.props.consignmentStatus == "PAYMENT_PENDING" && (
                    <React.Fragment>
                        <div className={styles.commonTitle}>
                            ...
                            <span className={styles.estimatedDate}>Your payment is in process</span>
                        </div>
                ...
                {this.props.isGiveAway === "N" &&
                    ...
                    this.props.deliveryDate && (
                        <div className={styles.deliveryDate}>
                            Delivered on: <span className={styles.estimatedDate}>{deliveryDateFormatted}</span>
                        </div>
                    )}
                {this.props.installationRequestCancelled &&
                    this.props.installationRequestCancelled.value.status === "Completed" && (
                        <div className={styles.deliveryDate}>
                ...
                {this.props.installationCompletedDate &&
                    ...
                    this.props.installationRequestCompleted.value.customerFacingName === "Request Completed" &&
                    this.props.installationRequestCompleted.value.status === "Completed" && (
                        <div className={styles.deliveryDate}>
                            Installation Completed On:{" "}
                            <span className={styles.estimatedDate}>{this.props.installationCompletedDate}</span>
                        </div>
                    )}
                {this.props.installationRequestReschedule &&
                    this.props.installationRequestReschedule.value.status === "Completed" && (
                        <div className={styles.commonTitle}>
                            <span className={styles.ffsemibold}>Installation Rescheduled On: </span>
                ...
                {this.props.installationRequestClosed &&
                    this.props.installationRequestClosed.value.status === "Completed" && (
                        <div className={styles.commonTitle}>
                            <span className={styles.ffsemibold}>Installation Closed On: </span>
                ...
                {this.props.estimatedCompletionDate && !this.props.hideEstimatedInstallationDate && (
                    <div className={styles.deliveryDate}>
                        ...
                        <span className={styles.estimatedDate}>{this.props.estimatedCompletionDate}</span>
                    </div>
                )}
                {this.props.isGiveAway === "N" &&
                    ...
                    date && (
                        <div className={styles.commonTitle}>
                            {!this.props.calloutMessage ? (
                                    ...
                                    {!this.props.isDigitalProduct &&
                                        ...
                                        (date || returnEligibleDate) && (
                                            <React.Fragment>
                                                ...
                                                {shipmentStatus.includes(EDD_TEXT) && estimatedDeliveryDateFormatted ? (
                                                ...
                                                {shipmentStatus &&
                                                    shipmentStatus.includes("Order Could be collected by") && (
                                                        <span className={styles.styleDate}>
                                                            {format(orderCouldbeCollected.toString(), dateFormat)}
                                                        </span>
                                                    )}
                                ...
                                <React.Fragment>
                                    {!this.props.calloutMessage.includes(EDD_TEXT) && (
                                        <div className={styles.commonTitle}>{this.props.calloutMessage}</div>
                                    )}
                                </React.Fragment>
                            )}
                        </div>
                ...
                {this.props.itemBreachMessage && (
                    <div className={styles.breachMessage}>* {this.props.itemBreachMessage}</div>
                )}
                {this.props.sellerName && <div className={styles.sellerName}>Sold By : {this.props.sellerName}</div>}
                {this.props.sshipLPUrl && (
                    <div ref={this.copyText} onClick={event => this.copyToClipBoard(event)}>
                        {this.props.sshipLPUrl}
                    </div>
                )}
                {/* <div
...
OrderCard.propTypes = {
    ...
    statusDisplayMsg: PropTypes.arrayOf(),
    ...
    onHollow: PropTypes.bool,
    children: PropTypes.node,
    returnFlow: PropTypes.bool,
    imageHolderWidth: PropTypes.string,
    imageHolderHeight: PropTypes.string,
    imageUrl: PropTypes.string,
    egvCardNumbe: PropTypes.string,
    ...
    returnStoreAddress: PropTypes.shape({
    ...
    returnModeSelected: PropTypes.string,
    ...
    showRightArrow: PropTypes.string,
    orderBreachMessage: PropTypes.string,
    showQuantity: PropTypes.bool,
    idFromAllOrderDetails: PropTypes.string,
    numberOfQuantity: PropTypes.number,
    quantity: PropTypes.bool,
    sshipLPUrl: PropTypes.string,
    itemBreachMessage: PropTypes.string,
    isDigitalProduct: PropTypes.bool,
    estimatedCompletionDate: PropTypes.string,
    hideEstimatedInstallationDate: PropTypes.bool,
    installationRequestClosed: PropTypes.shape({
        value: PropTypes.shape({
            status: PropTypes.string,
            date: PropTypes.string,
        }),
    }),
    installationRequestReschedule: PropTypes.shape({
        value: PropTypes.shape({
            status: PropTypes.string,
            date: PropTypes.string,
        }),
    }),
    installationRequestCompleted: PropTypes.shape({
        value: PropTypes.shape({
            status: PropTypes.string,
            date: PropTypes.string,
            customerFacingName: PropTypes.string,
        }),
    }),
    installationRequestCancelled: PropTypes.shape({
        value: PropTypes.shape({
            status: PropTypes.string,
            date: PropTypes.string,
        }),
    ...
    exchangeDetails: PropTypes.shape({
        ...
        exchangeModelName: PropTypes.bool,
    }),
    bundledAssociatedItems: PropTypes.arrayOf(),
    returnComments: PropTypes.string,
    returnSubReason: PropTypes.string,
    ...
    showIsGiveAway: PropTypes.bool,
    orderCancelDate: PropTypes.string,
    displayStatusName: PropTypes.string,
    retryPayment: PropTypes.string,
    ...
    installationCompletedDate: PropTypes.string,
    ...RouterPropTypes,
};
