...
export default class CustomerQueryForm extends Component {
        ...
        this.state = {
            ...
            mobile:
                getUserDetails && getUserDetails.loginType === "mobile" && getUserDetails.userName
                    ...
                    : "",
            email:
                getUserDetails && getUserDetails.loginType === "email" && getUserDetails.userName
                    ? getUserDetails.userName
                    : "",
            basicForm: true,
            attachment: false,
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps && nextProps.customerQueriesField !== this.props.customerQueriesField) {
            this.initialState(nextProps.customerQueriesField, true);
        }
        if (nextProps && nextProps.userDetails !== this.props.userDetails) {
            this.setState({
                email: nextProps.userDetails.emailID ? nextProps.userDetails.emailID : "",
                name:
    ...
    componentDidMount() {
        ...
        if (!this.props.formSubmit) {
            if (this.props.questionType == "orderRelated") {
                setDataLayerForCLiQCarePage(ADOBE_SELF_SERVE_PAGE_LOAD, this.getOrderData(), "Care_Order_Webform_1");
            }
        ...
        if (this.props.userDetails) {
            this.setState({
                email: this.props.userDetails.emailID ? this.props.userDetails.emailID : "",
                name:
    ...
    getOrderData = () => {
        ...
        return {
            status: selectedOrder && selectedOrder.products && selectedOrder.products[0].statusDisplay,
            id: selectedOrder && selectedOrder.products && selectedOrder.products[0].transactionId,
            productId: selectedOrder && selectedOrder.products && selectedOrder.products[0].productcode,
        };
    };
        ...
        customerQueriesField &&
            customerQueriesField.map(field => {
                if (field.componentName === "attachmentComponent") {
                    ...
                    if (field.componentName === "radioComponent" || field.componentName === "checkboxComponent") {
        ...
        return (
            ...
            this.props.customerQueriesField.map(listOfField => {
                ...
                if (listOfField.componentName == "textboxComponent") {
                    return (
                        <div className={this.state.isRadioComponent ? styles.txtFieldBox : styles.txtFieldBoxMargin}>
                            <div className={styles.txtFieldHeading}>
                                {listOfField.isMandatory == 1 ? listOfField.heading + "*" : listOfField.heading}
                            </div>
                ...
                if (listOfField.componentName == "radioComponent") {
                    return (
                        <div className={styles.radioComponentBox}>
                            <div className={styles.txtFieldHeading}>
                                {listOfField.isMandatory ? listOfField.heading + " *" : listOfField.heading}
                            </div>
                                ...
                                listOfField.optionArray.map(ele => {
                                    return (
                                        <CheckboxAndText
                                            ...
                                            size={"18px"}
                                            fontSize={"12px"}
                                            selected={this.state[listOfField.componentId] === ele.value ? true : false}
                                            selectItem={() => this.setState({ [listOfField.componentId]: ele.value })}
                                        />
                                    );
                                })}
                        </div>
    ...
    nextField(currentStep) {
        ...
        const { attachementData, mobile, email, uploadFileTitle, uploadedAttachment, name } = this.state;

        let validateStatus = false;
        if (currentStep == BASIC_FORM) {
            if (this.props.questionType == "orderRelated") {
                setDataLayerForCLiQCarePage(ADOBE_SELF_SERVE_PAGE_LOAD, this.getOrderData(), "Care_Order_Webform_2");
            }
            if (this.props.questionType == "NonOrderRelated") {
                setDataLayerForCLiQCarePage(
                    ADOBE_SELF_SERVE_NON_ORDER_PAGE_LOAD,
            ...
            if (validateStatus) {
                this.setState({
                    ...
                    attachment: true,
                    btnDisable:
                        attachementData && attachementData.isMandatory && this.state.file.length == 0 ? true : false,
        ...
        if (currentStep == ATTACHEMENT) {
            if (this.props.questionType == "orderRelated") {
                setDataLayerForCLiQCarePage(ADOBE_SELF_SERVE_PAGE_LOAD, this.getOrderData(), "Care_Order_Webform_3");
            }
            if (this.props.questionType == "NonOrderRelated") {
                setDataLayerForCLiQCarePage(
                    ADOBE_SELF_SERVE_NON_ORDER_PAGE_LOAD,
            ...
            if (this.state.mobile && !MOBILE_PATTERN.test(mobile)) {
                ...
                let ticketInfo = Object.assign(
                    {},
                    {
                        ...
                        issueBucket: this.props.parentIssueType ? this.props.parentIssueType : "",
                        issueType: question.issueType ? question.issueType : question.subIssueType,
                        transactionId:
                            this.props.questionType == "orderRelated"
                                ...
                                : "",
                        orderCode:
                            this.props.questionType == "orderRelated" ? selectedOrder && selectedOrder.orderId : "",
                        subOrderCode:
                            this.props.questionType == "orderRelated"
                                ? selectedOrder &&
                                  selectedOrder.products &&
                ...
                if (combinedSize <= maxFileSize && totalFile.length <= maxFileLimit) {
                    ...
                    if (uploadFileResponse && status === SUCCESS) {
                        this.setState(prevState => ({
                            ...
                            btnDisable: false,
                            uploadedAttachment: [...prevState.uploadedAttachment, ...uploadUserFile.imageURLlist],
                        }));
                    }
                } else {
                    if (totalFile.length > maxFileLimit)
                        this.props.displayToast(`Maximum ${maxFileLimit} No. of files allowed`);
                    else this.props.displayToast(`File size should be less then ${maxFileSize} MB`);
                }
            }
        } else {
    ...
    render() {
        const { basicForm, attachment, communication, currentStep, btnLabel } = this.state;
        let strArr = null;
        return (
            <div className={styles.base}>
                <div className={styles.headerBox}>
                    ...
                    {communication && <div className={styles.header}>{"Communication details"}</div>}

                    <div className={styles.buttonBox}>
                        <div className={styles.customBtn} onClick={() => this.previewPage()}>
                            Go to Previous Page
                        </div>
                    </div>
                </div>
                {basicForm && <div className={styles.basicForm}>{this.formField()}</div>}
                {attachment && (
                    <div className={styles.attachment}>
                        <div className={[styles.fileBox, this.state.file.length ? styles.borderFile : null].join(" ")}>
                            <div className={styles.fileInstruction}>
                                <div className={styles.uploadHeding}>{this.state.attachementData.heading}</div>
                                <div className={styles.uploadSubHeding}>{this.state.attachementData.itemsTitle}</div>
                            </div>
                            <div className={styles.fileUpload}>
                                ...
                                <input
                                    ...
                                    onChange={e => this.onUploadFile(e.target.files, this.state.attachementData)}
                                    title=""
                                    multiple={true}
                                    accept="application/pdf, image/*" // accepting only pdf/images(all types)
                                />
                                <div className={styles.fileSize}>
                                    {`Upload JPEG, PNG or PDF (Maximum size ${this.state.attachementData &&
                        ...
                        <div className={styles.filesBox}>
                            {this.state.file &&
                                this.state.file.map((file, index) => {
                                    ...
                                    return (
                                        <div key={`${file.name}_${index}`} className={styles.uploadFilesBox}>
                                            <div className={styles.deleteBOx} onClick={() => this.deleteFile(index)}>
                ...
                <div className={styles.formAction}>
                    <div className={styles.stepIndicator}>
                        <span className={[styles.steps, basicForm ? styles.currentStep : null].join(" ")} />
                        <span className={[styles.steps, attachment ? styles.currentStep : null].join(" ")} />

                        <span className={[styles.steps, communication ? styles.currentStep : null].join(" ")} />
                    </div>
