        ...
        return (
            <div className={Styles.base}>
                <div className={Styles.whiteCard}>
                    <div className={Styles.headerBox}>
                        <div className={Styles.header}>
                            {" "}
                            {this.props.isRecentOrderDetails ? "Ticket Details" : "All Tickets"}
                        </div>
                        {this.props.isRecentOrderDetails && (
                            <div className={Styles.previousPageBtn} onClick={() => this.props.navigatePreviousPage()}>
                                Go to Previous Page
                            </div>
                        )}
                        {!this.props.isRecentOrderDetails && (
                        ...
                        {this.props.filterCard && (
                            <div className={Styles.filterCard}>
                                {ticketStatusDropDownMenu.map((filterData, index) => {
                                    return (
                                        <div
                                            key={index}
                                            className={Styles.filterType}
                                            onClick={() => this.props.handleSelectedFilterClick(filterData)}
                                        >
                                            {filterData.label}
                                        </div>
                                    );
                    ...
                    <div className={Styles.contentBox}>
                        ...
                        ticketHistoryDetails.tickets.length > 0 ? (
                            <div>
                                {ticketHistoryDetails.tickets.map((tickets, index) => {
                                    return (
                                        <div
                                            key={index}
                                        ...
                                        >
                                            <div className={tickets.issueBucket ? Styles.nonOrderImg : Styles.orderImg}>
                                                {tickets.issueBucket ? (
                                            ...
                                            <div className={Styles.orderDetails}>
                                                <div className={Styles.productName}>{tickets.issueType}</div>
                                                <div className={Styles.fontLight}>
                                                    ...
                                                    <span className={Styles.fontBold}>
                                                        {tickets.status} {tickets.status === RESOLVED ? "|" : null}{" "}
                                                        {tickets.status === RESOLVED &&
                            ...
                            !this.props.isRecentOrderDetails && (
                                <div className={Styles.noTicket}> You have no tickets matching selected criteria </div>
                            )
                        )}
                        {this.props.isRecentOrderDetails && (
                            <OrderHistoryDetails
                ...
                {!this.props.isRecentOrderDetails &&
                    ...
                    Array.isArray(ticketHistoryDetails.tickets) &&
                    ticketHistoryDetails.ticketCount >= ticketHistoryDetails.tickets.length + 1 && (
                        <div className={Styles.showMoreButtonBox}>
                            <div className={Styles.showMore} onClick={() => this.props.loadMoreData()}>
                                Show More
                            </div>
                        </div>
                    )}
...
OrderHistoryList.propTypes = {
    ...
    selectedTickerHistory: PropTypes.any,
    showRecentOrderDetails: PropTypes.showRecentOrderDetails,
    ticketHistoryDetails: PropTypes.shape({
