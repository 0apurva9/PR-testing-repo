...
import styles from "./ReturnToStore.css";
...
export default class ReturnToStore extends React.Component {
        ...
        if (checkUserAgentIsMobile()) {
            this.setState({ storeId }, () => {
                this.props.history.push({
                    pathname: `${RETURNS_PREFIX}/${this.orderCode}${RETURN_TO_STORE}${RETURNS_STORE_FINAL}`,
                    state: {
    ...
    navigateToReturnLanding() {
        return <Redirect to={`${RETURNS_PREFIX}/${this.orderCode}${RETURN_LANDING}${RETURNS_REASON}`} />;
    }

    quickDropStore = pincode => {
        this.setState({ pincode, isStoreSelected: false });
        if (pincode.length === 6) {
            this.props.quickDropStore(pincode, this.props.returnProductDetails.orderProductWsDTO[0].USSID);
        }
    };
        ...
        if (this.state.storeId) {
            if (this.props.isCOD) {
                ...
                if (this.state.isStoreSelected) {
                    ...
                    if (this.props.bankDetail.accountNumber !== this.props.bankDetail.reEnterAccountNumber) {
                    ...
                    if (this.props.bankDetail.code && !IFSC_PATTERN.test(this.props.bankDetail.code)) {
    ...
    getStoreDetails = val => {
        if (val.length > 0) {
            let selectedStore =
                this.props.returnRequest &&
                this.props.returnRequest.returnStoreDetailsList.find(store => store.slaveId === val);
    ...
    render() {
        ...
        if (!this.props.location.state || !this.props.location.state.authorizedRequest) {
            return this.navigateToReturnLanding();
        }
        let noOfStories = 0;

        if (this.props.returnRequest && this.props.returnRequest.returnStoreDetailsList) {
            noOfStories = this.props.returnRequest.returnStoreDetailsList.length;
        }

        if (!this.props.returnRequest || !this.props.returnProductDetails || this.props.loading) {
        ...
        const renderStoresMap = (
            <React.Fragment>
                <MobileOnly>
                    <PiqPage
                        {...this.props}
                        productName={this.props.returnProductDetails.orderProductWsDTO[0].productName}
                        productColour={this.props.returnProductDetails.orderProductWsDTO[0].productColour}
                ...
                <DesktopOnly>
                    <CliqAndPiqMap
                        availableStores={this.props.returnRequest && this.props.returnRequest.returnStoreDetailsList}
                        lat={this.state.lat}
                        lng={this.state.lng}
                    />
                        ...
                        !this.props.showPickupPerson && (
                            <div className={styles.addressDetail}>
                                <DumbGrid limit={1} offset={0} elementWidthDesktop={100}>
                                    {this.props.returnRequest.returnStoreDetailsList.map((val, i) => {
                                        return (
                                            <PickUpLocation
                                                key={i}
                                                slaveId={val.slaveId}
                                                ...
                                                handleClickForDesktop={val => this.getStoreDetails(val)}
        ...
        return (
            <div className={styles.base}>
                <DesktopOnly>
                    ...
                    {this.state.isStoreSelected && returnAddressDetails && (
                        <React.Fragment>
                            <SelectedReasonForReturn
                                header={"Select mode of return "}
                                title={returnAddressDetails.displayName}
                                titleDescription={`${returnAddressDetails.address.line1}, ${returnAddressDetails.address.line2}`}
                    ...
                    , ${returnAddressDetails.address.postalCode}`}
                                date={
                                    returnAddressDetails.mplWorkingDays === "7"
                                        ? "all days"
                                        : returnAddressDetails.mplWorkingDays.split("").map(val => {
                                              if (val !== ",") {
                                ...
                                time={`${returnAddressDetails.mplOpeningTime} - to ${returnAddressDetails.mplClosingTime} Hrs`}
