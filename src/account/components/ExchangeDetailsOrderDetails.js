...
export default class ExchangeDetailsOrderDetails extends React.Component {
    ...
    render() {
        ...
        if (exchangeCanceled && exchangeCanceled.status && exchangeCanceled.status === "Complete") {
            // hideExchangeDetails = true;
        }
        if (cashbackCredited && cashbackCredited.status && cashbackCredited.status === "Complete") {
            // hideExchangeDetails = true;
        ...
        if (
            ...
            Object.keys(this.props.products.exchangeDetails.exchangeTrackDiagram[0]).length === 0
        ) {
        ...
        return (
            <div className={styles.exchangeDetailsContainer}>
                ...
                <div className={styles.exchangeProductText}>
                    ...
                    <span
                        className={!this.state.isToggleOn ? styles.downArrowRotate : styles.downArrow}
                        onClick={() => this.toggle()}
                    />
                </div>
                ...
                {!this.state.isToggleOn ? (
                    <React.Fragment>
                        {this.props.products.exchangeDetails.exchangePriceDetail && (
                            <React.Fragment>
                                <table className={styles.exchangePricingDetails} cellPadding={0} cellSpacing={0}>
                                    <tbody>
                                        {this.props.products.exchangeDetails.exchangePriceDetail
                                        ...
                                        {this.props.products.exchangeDetails.exchangePriceDetail.TULBump && (
                                            <tr>
                                        ...
                                        {this.props.products.exchangeDetails.exchangePriceDetail.pickupCharge && (
                                            <tr>
                                                <td>Pick Up Charge </td>
                                                {this.props.products.exchangeDetails.exchangePriceDetail.pickupCharge
                                                    .doubleValue === 0 && <td className={styles.pickupCharge}>FREE</td>}
                                                {this.props.products.exchangeDetails.exchangePriceDetail.pickupCharge
                                                    .doubleValue !== 0 && (
                                                    <td>
                                                        {
                                ...
                                {this.props.products.exchangeDetails.exchangePriceDetail.effectiveAmount && (
                                    <div className={styles.effectivePriceContainer}>
                                        <div className={styles.effectivePriceText}>
                                            <span className={styles.fontLight}>Effective Price for</span>{" "}
                    ...
                    !this.props.products.consignmentStatus.includes("DELIVERED") && (
                        <React.Fragment>
                            {!this.props.products.exchangeDetails.exchangePickupPromiseDate &&
                                !this.props.products.exchangeDetails.exchangePickedUpDate && (
                                    <div className={styles.exchangeEDDContainer}>
                                        ...
                                        <span className={styles.fontLight}> Within 3 days of Product Delivery</span>
                                    </div>
                                )}
                            ...
                            {this.props.products.exchangeDetails.exchangePickupPromiseDate &&
                                !this.props.products.exchangeDetails.exchangePickedUpDate && (
                                    <div className={styles.exchangeEDDContainer}>
                                        <span className={styles.fontBold}>Estimated Exchange Pick up Date:</span>
                            ...
                            {this.props.products.exchangeDetails.exchangePickedUpDate && (
                                <div className={styles.exchangeEDDContainer}>
                                    <span className={styles.fontBold}>Exchange Product Picked up on:</span>
                                    <span className={styles.fontLight}>
                                        {" "}
                                        {format(this.props.products.exchangeDetails.exchangePickedUpDate, dateFormat)}
                                    </span>
                                </div>
                            )}
                            ...
                            {!this.props.products.exchangeDetails.exchangeTrackDiagram &&
                                ...
                                this.props.products.exchangeDetails.exchangePaymentDetails[0].exchangePaymentMode && (
                                    <div className={styles.exchangeCashbackDetails}>
                                        <div className={styles.exchangeCashbackTextContainer}>
                                            <span className={styles.exchangeCashbackText}>
                                                You will receive Exchange Cashback, post your old phone pickup, in{" "}
                                            </span>
                                            {this.props.products.exchangeDetails.exchangePaymentDetails[0]
                ...
                {cashbackCredited && cashbackCredited.status && cashbackCredited.status === "Complete" && (
                    <React.Fragment>
                        {this.props.products.exchangeDetails.exchangePaymentDetails &&
                            this.props.products.exchangeDetails.exchangePaymentDetails[0] && (
                        ...
                        <div className={styles.exchangeProcessedText}>Exchange has been processed sucessfully.</div>
                    </React.Fragment>
                )}
                {!this.props.products.consignmentStatus.includes("CANCEL") &&
                    ...
                    !hideTrackDiagram && (
                        <ExchangeDetailsTrack
                            exchangeTrackDiagram={this.props.products.exchangeDetails.exchangeTrackDiagram}
                        />
                    )}
            </div>
        );
