...
import { CATEGORY_CAPTURE_REGEX, BRAND_CAPTURE_REGEX } from "./PlpBrandCategoryWrapper.js";
...
class ProductListingsPage extends Component {
     ...
     */
    categoryQueryStringURL(searchText, textToReplace, replaceWith, categoryCodeParameter) {
    ...
    getSearchTextFromUrl(currentUrl) {
        ...
        if (searchCategory && searchCategory !== "" && searchCategory !== SEARCH_CATEGORY_TO_IGNORE) {
        ...
        if (this.props.match.path === SKU_PAGE) {
            if (searchText) {
                if (searchText.includes("relevance")) {
                    ...
                    !searchText.includes(`:price-asc:collectionIds:${this.props.match.params.slug}`)
                ) {
                    searchText = searchText.replace(
                        ":price-asc",
                    ...
                    !searchText.includes(`:price-desc:collectionIds:${this.props.match.params.slug}`)
                ) {
                    searchText = searchText.replace(
                        ":price-desc",
        ...
        if (
            ...
            if (searchText) {
                ...
                } else if (searchText.includes("price-asc") && !searchText.includes(`${CATEGORY_TEXT}:${catParams}`)) {
                    searchText = searchText.replace(":price-asc", `:price-asc:${CATEGORY_TEXT}:${catParams}`);
                } else if (searchText.includes("price-desc") && !searchText.includes(`${CATEGORY_TEXT}:${catParams}`)) {
                    searchText = searchText.replace(":price-desc", `:price-desc:${CATEGORY_TEXT}:${catParams}`);
                }
            } else {
        ...
        if (
            ...
            if (searchText) {
                if (searchText.includes("relevance")) {
                    searchText = searchText.replace(":relevance", `:relevance:brand:${brandParams}`);
                } else if (
                    searchText.includes("price-asc") &&
                    !searchText.includes(`:price-asc:brand:${brandParams}`)
                ) {
                    searchText = searchText.replace(":price-asc", `:price-asc:brand:${brandParams}`);
                } else if (
                    searchText.includes("price-desc") &&
                    !searchText.includes(`:price-desc:brand:${brandParams}`)
                ) {
                    searchText = searchText.replace(":price-desc", `:price-desc:brand:${brandParams}`);
                }
            } else {
    ...
    componentDidMount() {
        ...
        if (this.props.location.state && this.props.location.state.disableSerpSearch === true) {
            return;
        }
        if (isBrowser) {
            if (!this.props.urlString && this.props.lastVisitedPlpUrl === window.location.href) {
                if (this.props.clickedProductModuleRef) {
                    const clickedElement = document.getElementById(this.props.clickedProductModuleRef);
                    if (clickedElement) {
                        delay(() => {
                ...
                if (
                    ...
                    (this.props.lastVisitedPlpUrl.includes("icid2") || this.props.lastVisitedPlpUrl.includes("cid"))
                ) {
                    const search = queryString.parse(this.props.location && this.props.location.search);
                    if (search.icid2) {
                        icid = search.icid2;
                        icidType = ICID2;
                    } else if (search.cid) {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.urlString) {
            ...
            if (
                ...
                if (this.props.urlString.includes("https") || this.props.urlString.includes("http")) {
        ...
        if (this.props.location.search !== prevProps.location.search) {
            ...
            page = 0;
            if (this.props.location.state && this.props.location.state.isFilter === true) {
                const suffix = "&isFilter=true";
                const searchText = this.getSearchTextFromUrl();
                const pageMatch = PAGE_REGEX.exec(this.props.location.pathname);
                if (pageMatch) {
                ...
                this.props.getProductListings(searchText, suffix, page, true);
                return;
            } else if (this.props.location.state && this.props.location.state.isFilter === false) {
                const searchText = this.getSearchTextFromUrl();
                const pageMatch = PAGE_REGEX.exec(this.props.location.pathname);
                if (pageMatch) {
                    page = pageMatch[1] ? pageMatch[1] : 1;
