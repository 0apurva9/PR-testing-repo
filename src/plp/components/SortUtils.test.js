...
it("should work with a brand page that has a query", () => {
  let url = `/footwear-men-sneakers/c-${DUMMY_BRAND_VALUE}`;
  let endUrl = applySortToUrl(
    ":relevance:inStockFlag:true:isLuxuryProduct:false",
    url,
    ARRAY_OF_SORTS[3],
  ...
  expect(endUrl).toEqual(
    `/search/?q=:${
      ARRAY_OF_SORTS[3]
    }:inStockFlag:true:isLuxuryProduct:false:brand:${DUMMY_BRAND_VALUE.toUpperCase()}&icid2=auto:mul:msd:MSH1311115:all:none:00:030518`
  );
});
