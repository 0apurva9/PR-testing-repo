...
export const SECONDARY_FEED_COMPONENT_DATA_SUCCESS = "SECONDARY_FEED_COMPONENT_DATA_SUCCESS";
export const SECONDARY_FEED_COMPONENT_DATA_FAILURE = "SECONDARY_FEED_COMPONENT_DATA_FAILURE";
export const SECONDARY_FEED_COMPONENT_DATA_REQUEST = "SECONDARY_FEED_COMPONENT_DATA_REQUEST";

export const SECONDARY_FEED_GET_ITEMS_REQUEST = "SECONDARY_FEED_GET_ITEMS_REQUEST";
export const SECONDARY_FEED_GET_ITEMS_FAILURE = "SECONDARY_FEED_GET_ITEMS_FAILURE";
export const SECONDARY_FEED_GET_ITEMS_SUCCESS = "SECONDARY_FEED_GET_ITEMS_SUCCESS";
...
export const AUTO_WISHLIST_COMPONENT_SUCCESS = "AUTO_WISHLIST_COMPONENT_SUCCESS";
export const AUTO_WISHLIST_COMPONENT_REQUEST = "AUTO_WISHLIST_COMPONENT_REQUEST";
...
export const CLEAR_ITEMS_FOR_PARTICULAR_POSITION = "CLEAR_ITEMS_FOR_PARTICULAR_POSITION";
...
const MSD_AUTOMATED_BRAND_CAROUSEL = "msdAutomatedBannerProductCarouselComponent";
...
const AUTO_PRODUCT_RECOMMENDATION_COMPONENT = "Auto Product Recommendation Component";
    ...
    return async (dispatch, getState, { api }) => {
        ...
        try {
            const result = await api.post(
                `v2/mpl/users/${JSON.parse(userDetails).userName}/getProductCapsules?access_token=${
...
export function getItems(positionInFeed, itemIds, feedType) {
    return async (dispatch, getState, { api }) => {
        ...
        try {
            ...
            let requests =
                itemIds &&
                itemIds.map(id => api.getMiddlewareUrl(`v2/mpl/cms/page/getProductInfo?isPwa=true&productCodes=${id}`));
            ...
            if (feedType === SECONDARY_FEED_TYPE) {
                dispatch(secondaryFeedGetItemsSuccess(positionInFeed, productList, itemIds));
            } else {
...
export function homeFeedBackUp() {
    return async (dispatch, getState, { api }) => {
        ...
        try {
            ...
            dispatch(homeFeedBackupSuccess(resultJson));
        } catch (e) {
            ...
            if (sessionMbox) {
                ...
                mboxSessionIdJson =
                    splitData && splitData[1].includes("session")
                        ...
                        : splitData[0].split("#")[1];
                tntId =
                    splitData && splitData[1].includes("PC") ? splitData[1].split("#")[1] : splitData[0].split("#")[1];
...
export function getFeed(pageId) {
    return async (dispatch, getState, { api }) => {
        if (pageId) {
        ...
        try {
            let result, feedTypeRequest, resultJson;
            ...
            if (pageId) {
                ...
                if (resultJson.errors) {
                    ...
                    dispatch(
                        secondaryFeedSuccess(resultJson.items, resultJson.seo, resultJson.message, feedTypeRequest)
                    );
                    if (CATEGORY_REGEX.test(pageId)) {
                        setDataLayer(ADOBE_CLP_PAGE_LOAD, resultJson, getState().icid.value, getState().icid.icidType);
                    } else if (BRAND_REGEX.test(pageId)) {
                        setDataLayer(ADOBE_BLP_PAGE_LOAD, resultJson, getState().icid.value, getState().icid.icidType);
                    } else if (pageId === window.location.pathname.replace("/", "")) {
                        setDataLayer(ADOBE_STATIC_PAGE, resultJson, getState().icid.value, getState().icid.icidType);
                    }
                }
            } else {
                ...
                // } else if (process.env.environment === "p2") {
            ...
            if (!resultJson || !resultJson.content || resultJson.status === "FAILURE") {
...
export function secondaryFeedComponentDataSuccess(data, positioninFeed, isMsd) {
    return {
...
export function componentDataSuccess(data, positionInFeed, isMsd) {
    return {
...
export function getComponentData(positionInFeed, fetchURL, postParams, backUpUrl, type, feedType, msdABPCBrandCount) {
    return async (dispatch, getState, { api }) => {
        ...
        try {
            ...
            if (postParams && postParams.widgetPlatform === MSD_WIDGET_PLATFORM) {
                ...
                postData = await getMsdFormData(widgetSpecificPostData.widget_list, [msdNumberOfResults]);
                ...
                if (type === DISCOVER_MORE || type === AUTO_PRODUCT_RECOMMENDATION_COMPONENT || MSD_DISCOVER_MORE) {
                    postData.set("num_results", JSON.stringify([DISCOVER_MORE_NUM_RESULTS]));
                }
                ...
                if (feedType === SECONDARY_FEED_TYPE) {
                    dispatch(secondaryFeedComponentDataSuccess(resultJson, positionInFeed, true));
                } else {
...
export function msdAbcComponents() {
    return async (dispatch, getState, { api }) => {
        try {
            ...
            postData = await getMsdFormData(AUTOMATED_BRAND_CAROUSEL_WIDGET_LIST, [msdNumberOfResults]);
...
export function msdDiscoverMoreHomeComponents() {
    return async (dispatch, getState, { api }) => {
        try {
            // const userDetails = Cookie.getCookie(LOGGED_IN_USER_DETAILS);
...
export function automatedWidgetsForHomeSuccess(homeAutoWidgetData, widgetKey, productCode, filterData) {
    return {
    ...
    return async (dispatch, getState, { api }) => {
        try {
            ...
            if (resultJson && resultJson.status === "Success" && resultJson.results) {
                dispatch(automatedWidgetsForHomeSuccess(resultJson.results, widgetKey, productCode));
            }
        } catch (e) {
            throw new Error(`${e.message}`);
        }
...
export function automatedWidgetsForHome(widgetData) {
    return async (dispatch, getState, { api }) => {
        try {
            ...
            let productId = widgetData && widgetData.hexCode && widgetData.hexCode.toUpperCase();
            ...
            if (widgetData && widgetData.description) {
                let filterValue = widgetData && widgetData.description && widgetData.description.split(";");
                let filterParsedData;
                if (filterValue && Array.isArray(filterValue) && filterValue[1] === "range") {
                    let rangeValue = filterValue && filterValue[2] && filterValue[2].split(",");
                    filterParsedData = [
                        {
                            field: `${filterValue[0]}`,
                            type: `${filterValue[1]}`,
            ...
            const msdWidgetDataResult = await api.postMsd(`${MSD_ROOT_PATH}/widgets`, msdWidgetData);
            ...
            if (widgetData.webURL === "114") {
                data = msdWidgetDataJson && msdWidgetDataJson.data[0] && msdWidgetDataJson.data[0].itemIds;
            } else {
            ...
            if (
                ...
                // } else {
                dispatch(
                    automatedWidgetsForHomeSuccess(data, widgetData.webURL, widgetData.hexCode, widgetData.description)
                );
                //}
            }
        } catch (e) {
