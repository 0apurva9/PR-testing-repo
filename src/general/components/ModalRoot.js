...
const NonEmiEligibleToWishlistContainer = Loadable({
    loader: () => import("../../cart/containers/NonEmiEligibleToWishlistContainer"),
    loading() {
        return <Loader />;
    },
});
...
const KycApplicationFormWithBottomSlideModal = Loadable({
    loader: () => import("../../account/components/KycApplicationFormWithBottomSlideModal"),
    loading() {
        return <Loader />;
    },
});
...
const KycDetailPopUpWithBottomSlideModal = Loadable({
    loader: () => import("../../account/components/KycDetailPopUpWithBottomSlideModal"),
    loading() {
        return <Loader />;
    },
});
...
const SellerReviewSubmitRemovalPopup = Loadable({
    loader: () => import("../../account/components/SellerReviewSubmitRemovalPopup.js"),
    loading() {
        return <Loader />;
    },
});
...
const AttachmentUploadPopUp = Loadable({
    loader: () => import("../../account/components/attchment-upload-response-popup.jsx"),
    loading() {
        return <Loader />;
    },
});
...
const RatingAndReviewWrapperModalV2 = Loadable({
    loader: () => import("../../pdp/components/RatingAndReviewWrapperModalV2.js"),
    loading() {
        return <Loader />;
    },
});
...
export default class ModalRoot extends React.Component {
    ...
    releasePreviousAndApplyNewBankOffer = (previousCouponCode, newSelectedCouponCode) => {
        this.props.releasePreviousAndApplyNewBankOffer(previousCouponCode, newSelectedCouponCode);
    };
    ...
    continueWithoutBankCoupon = async () => {
        ...
        if (this.props.ownProps && this.props.ownProps.couponCode) {
            if (this.props.ownProps.couponCode === bankCouponCode) {
                ...
                const releaseCouponCode = await this.props.releaseUserCoupon(userCouponCode);
                if (releaseCouponCode.status === SUCCESS) {
                ...
                const releaseCouponCode = await this.props.removeNoCostEmi(noCostEmiCoupon, carGuId, cartId);
                if (releaseCouponCode.status === SUCCESS) {
                    this.props.ownProps.redoCreateJusPayApi();
                    this.props.hideModal();
                }
            ...
            Promise.all([
                ...
                noCostEmiCoupon && this.props.removeNoCostEmi(noCostEmiCoupon, carGuId, cartId),
            ]).then(() => {
                localStorage.removeItem(BANK_COUPON_COOKIE);
                localStorage.removeItem(COUPON_COOKIE);
                this.props.ownProps.redoCreateJusPayApi();
    ...
    verifyIMEINumber(IMEINumber, exchangeProductId, exchangeAmountCashify, tulBump, pickUpCharge, listingId, ussId) {
        ...
        const MODAL_COMPONENTS = {
            ...
            UpdateProfileOtpVerification: (
                <OtpVerification
                    ...
                    resendOtp={() => this.resendOtpForUpdateProfile(this.props.ownProps)}
                    onClickWrongNumber={() => this.handleClose()}
                />
            ),
            ...
            ShowDeliveryConfirmPopup: <ShowDeliveryConfirmPopup closeModal={() => this.handleClose()} />,
            ...
            Sort: <Sort />,
            Address: <AddressModalContainer closeModal={() => this.handleClose()} {...this.props.ownProps} />,
            ...
            ConnectDetails: <ConnectDetailsWithModal closeModal={() => this.handleClose()} {...this.props.ownProps} />,
            Coupons: (
                <ProductCouponDetails
                    closeModal={() => this.handleClose()}
                    applyUserCoupon={couponCode => this.applyUserCoupon(couponCode)}
                    releaseUserCoupon={(couponCode, newCouponCode) => this.releaseUserCoupon(couponCode, newCouponCode)}
                    {...this.props.ownProps}
                    navigateToLogin={url => this.navigateToLogin(url, { ...this.props.ownProps })}
                />
            ),
            GoToCartPagePopUp: (
                <GoToCartPopUp
            ...
            BankOffers: (
                <BankOffersDetails
                    ...
                    releaseBankOffer={(couponCode, newCouponCode) => this.releaseBankOffer(couponCode, newCouponCode)}
                    {...this.props.ownProps}
            ...
            generateOtpForCliqCash: (
                <KycApplicationFormWithBottomSlideModal
                    ...
                    {...this.props.ownProps}
                    loadingForGetOtpToActivateWallet={this.props.loadingForGetOtpToActivateWallet}
                    displayToast={message => this.props.displayToast(message)}
                />
            ),
            verifyOtpForCliqCash: (
                <KycDetailPopUpWithBottomSlideModal
                    ...
                    submitOtp={otpDetails => this.verifyOtpForCliqCash(otpDetails, this.props.ownProps)}
                    {...this.props.ownProps}
            ...
            SizeSelectorForEyeWear: <SizeSelectorForEyeWear closeModal={() => this.handleClose()} />,

            StoryModal: <StoryWidgetContainer closeModal={() => this.handleClose()} {...this.props.ownProps} />,
            GenerateOtpForEgv: (
                <KycApplicationFormWithBottomSlideModal
                    closeModal={() => this.handleClose()}
            ...
            cashBackDetailsPopup: (
                <CashBackDetailsPopupContainer data={this.props.ownProps} closeModal={() => this.handleClose()} />
            ),
            cliqcashknowmore: <CliqCashKnowMore {...this.props.ownProps} closeModal={() => this.handleClose()} />,
            datePickerModule: (
                <DatePickerModule
                    {...this.props.ownProps}
                    closeModal={() => this.handleClose()}
            ...
            ManufacturerModal: (
                <ManufacturerDetailsModal closeModal={() => this.handleClose()} {...this.props.ownProps} />
            ),
            ReviewGuidelineWrapperModal: (
                <ReviewGuidelineWrapperModal {...this.props.ownProps} closeModal={() => this.handleClose()} />
            ),
            ...
            OfferDetailsModal: <OfferDetailsModal closeModal={() => this.handleClose()} {...this.props.ownProps} />,
            BeautyOfferDetailsModal: (
                <BeautyOfferDetailsModal closeModal={() => this.handleClose()} {...this.props.ownProps} />
            ),
            ...
            BundledProductModal: <BundledProductModal closeModal={() => this.handleClose()} {...this.props.ownProps} />,
            ...
            ExchangeModal: (
                <ExchangeModal
                    ...
                    addProductToCart={productDetails => this.props.addProductToCart(productDetails)}
                    {...this.props.ownProps}
                    history={this.props.history}
                    displayToast={this.props.displayToast}
                />
            ...
            ExchangeTnCModal: (
                <ExchangeTnCModal history={this.props.history} closeTnCModal={() => this.handleClose()} />
            ),
            NoCostEmiItemBreakUp: (
                <ItemLevelPopup emiItemDetails={this.props.ownProps} closeModal={() => this.handleClose()} />
            ),
            ...
            PriceBreakup: <PriceBreakupModal data={this.props.ownProps} closeModal={() => this.handleClose()} />,
            OrderModal: <OrderModal data={this.props.ownProps} closeModal={() => this.handleClose()} />,
            CancelOrderPopUp: (
                <CancelOrderPopUp
            ...
            UpiHowToPay: (
                <UpiHowToPay {...this.props} history={this.props.history} closeModal={() => this.handleClose()} />
            ),
            /**
             * EOD
             */
            ...
            CancelReturnRequestPopUp: (
                <CancelReturnRequestPopUp
                    data={this.props.ownProps}
                    loadingForUpdateReturnCancellation={this.props.loadingForUpdateReturnCancellation}
                    cancelModal={() => this.handleClose()}
                    updateReturnCancellation={data => this.updateReturnCancellation(data)}
                />
            ),
            SellerReviewSubmitRemovalPopup: (
                <SellerReviewSubmitRemovalPopup rating={this.props.rating} closeModal={() => this.handleClose()} />
            ),
            ExchangeRemoveModal: (
                <ExchangeRemoveModal
                    {...this.props.ownProps}
            ...
            AttachmentUploadPopUp: (
                <AttachmentUploadPopUp closeModal={() => this.handleClose()} {...this.props.ownProps} />
            ),
            ...
            RatingAndReviewWrapperModalV2: (
                <RatingAndReviewWrapperModalV2
                    ...
                    addReviewDetails={this.props.addReviewDetails}
                    ...
                    submitParameterRatingDetails={this.props.submitParameterRatingDetails}
