...
import { authCallsAreInProgress, stopLoaderOnLoginForOTPVerification } from "./auth.actions";
import * as ErrorHandling from "../../general/ErrorHandling.js";
import { displayToast, SOMETHING_WENT_WRONG } from "../../general/toast.actions";
...
export const FORGOT_PASSWORD_OTP_VERIFICATION_REQUEST = "FORGOT_PASSWORD_OTP_VERIFICATION_REQUEST";
export const FORGOT_PASSWORD_OTP_VERIFICATION_SUCCESS = "FORGOT_PASSWORD_OTP_VERIFICATION_SUCCESS";
export const FORGOT_PASSWORD_OTP_VERIFICATION_FAILURE = "FORGOT_PASSWORD_OTP_VERIFICATION_FAILURE";
...
export const SOCIAL_MEDIA_REGISTRATION_REQUEST = "SOCIAL_MEDIA_REGISTRATION_REQUEST";
export const SOCIAL_MEDIA_REGISTRATION_SUCCESS = "SOCIAL_MEDIA_REGISTRATION_SUCCESS";
export const SOCIAL_MEDIA_REGISTRATION_FAILURE = "SOCIAL_MEDIA_REGISTRATION_FAILURE";
...
export const FORGOT_PASSWORD_PATH = "v2/mpl/forgottenpasswordtokens/customerForgotPassword";
export const FORGOT_PASSWORD_OTP_VERIFICATION_PATH = "v2/mpl/forgottenpasswordtokens/forgotPasswordOTPVerification";
export const RESET_PASSWORD = "v2/mpl/forgottenpasswordtokens/forgotPassword?access_token=";
...
export const SOCIAL_MEDIA_REGISTRATION_PATH = "v2/mpl/users/socialMediaRegistration";
    ...
    try {
      ...
      if (resultJson.status && resultJson.status === "Success") {
        window.clevertap.profile.push({
          Site: {
            Email: resultJson && resultJson.customerInfo && resultJson.customerInfo.emailId,
        ...
        window.clevertap.onUserLogin.push({
          Site: {
            ...
            "MSG-sms": true, // Enable sms notifications
            "MSG-whatsapp": true, // Enable whatsapp notifications
            "test-Notification": `test check ${resultJson && resultJson.firstName}`,
          },
        });
      }
...
export function signUpUser(userObj) {
  ...
  return async (dispatch, getState, { api }) => {
    ...
    try {
      ...
      return dispatch(signUpUserSuccess(resultJson.customerId, userObj.emailId));
    } catch (e) {
  ...
  return async (dispatch, getState, { api }) => {
    ...
    try {
      const result = await api.post(
        ...
        }&otp=${otpDetails}&isPwa=true&platformNumber=${PLAT_FORM_NUMBER}&username=${userDetails.username}&password=${
...
export function resetPassword(userDetails) {
  ...
  return async (dispatch, getState, { api }) => {
    ...
    try {
      const url = `${RESET_PASSWORD}${JSON.parse(globalAccessToken).access_token}&platformNumber=${PLAT_FORM_NUMBER}`;
...
export function generateCustomerLevelAccessTokenForSocialMedia(userName, id, id_token, platForm, socialChannel) {
  return async (dispatch, getState, { api }) => {
    dispatch(customerAccessTokenRequest());
    try {
      const result = await api.post(
...
export function socialMediaRegistration(userName, id, accessToken, platForm, socialChannel) {
  let globalCookie = Cookie.getCookie(GLOBAL_ACCESS_TOKEN);
  return async (dispatch, getState, { api }) => {
    ...
    try {
      ...
      if (resultJsonStatus.status && resultJsonStatus.message !== EMAIL_ID_ALREADY_NOT_EXIST_SIGN_UP) {
        throw new Error(`${resultJsonStatus.message}`);
      }
...
export function socialMediaLogin(userName, platform, customerAccessToken, userDetailObj) {
  return async (dispatch, getState, { api }) => {
    ...
    try {
      ...
      return dispatch(socialMediaLoginSuccess(resultJson, platform, userDetailObj));
    } catch (e) {
      return dispatch(socialMediaLoginFailure(e.message, userName, platform, userDetailObj));
    }
  };
}
