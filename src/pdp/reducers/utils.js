import { EXPRESS, COLLECT, SHORT_COLLECT, SHORT_EXPRESS, HOME_DELIVERY } from "../../lib/constants";
...
export function renderComponent(componentItem, typeComponentMapping, props, componentDetailsObj) {
    if (!componentItem.componentId) {
        ...
        if (typeCompMapping) {
            return (
                ...
                typeComponentMapping[componentItem.componentId](props, componentDetailsObj)
            );
        } else {
            console.log("Missing component implementation", componentItem.componentId);
        }
    } else {
        return null;
    }
...
export function findSelectedSize(variantTheme = [], productListingId, fromColorComponent = false) {
    let sizeOptions = [];
