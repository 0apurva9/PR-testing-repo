...
import { setDataLayerForPdpDirectCalls, SET_DATA_LAYER_FOR_BUY_NOW_EVENT } from "../../lib/adobeUtils";
...
export default class SizeSelector extends React.Component {
    ...
    async updateSize(productUrl) {
        ...
        } else if (productCodeArray[2] && PRODUCT_CODE_REG_EX.test(productCodeArray[2])) {
            productCode = productCodeArray[2].substring(2);
        } else {
            productCode = this.props.productId;
        }
        if (this.props.isFromModal) {
            const productDetailResponse = await this.props.getProductDescription(productCode);
            if (productDetailResponse && productDetailResponse.status === SUCCESS) {
                ...
                if (this.props.buyNowFlag) {
                    ...
                    if (!checkUserLoggedIn()) {
                        localStorage.setItem(BUY_NOW_PRODUCT_DETAIL, JSON.stringify(productDetailsObj));
                    ...
                    let addToWishListObj = {
                        productListingId: productDescription && productDescription.productListingId,
    ...
    render() {
        ...
        const isEyeWearCategory =
            ...
            this.props.categoryHierarchy[0].category_name === "Eyewear"
                ? true
                : false;
        let positivePowerIndex = sizes && sizes.findIndex(val => val.sizelink.size > 0);
        let zeroPowerIndex = sizes && sizes.findIndex(val => val.sizelink.size === "0.00");

        let { showLensPower, label } = this.state;

        let isPowerLensSizeSelected = false,
            selectedLensSize = sizes && sizes.length ? sizes.filter(item => item.colorlink.selected) : [];
        if (
            this.props &&
            this.props.history &&
            this.props.history.location &&
            ...
            return (
                <div className={styles.size}>
                    ...
                    <DesktopOnly>
                        <div
                            className={
                                isEyeWearCategory && this.props.isSizeOrLength === "Power" && showLensPower
                        ...
                        >
                            ...
                            {isEyeWearCategory &&
                                ...
                                (showLensPower ? (
                                    ...
                                    <div
                                        className={
                                            isPowerLensSizeSelected ? styles.selectedPowerLens : styles.lensPowerLabel
                                        }
                                        onClick={() => this.showAllLensPower()}
                                        style={{
                                            padding:
                                                isPowerLensSizeSelected && selectedLensSize[0].sizelink.size > 0
                                                    ? "10px 0"
                                                    : null,
                                        }}
                                    >
                            ...
                            {this.props.eyeWearSizeGuide && isEyeWearCategory && this.props.hasSizeGuide && (
                                <DesktopOnly>
                                    <UnderLinedButton
                                        // disabled={!this.props.hasSizeGuide}
                                        label={SIZE_GUIDE}
