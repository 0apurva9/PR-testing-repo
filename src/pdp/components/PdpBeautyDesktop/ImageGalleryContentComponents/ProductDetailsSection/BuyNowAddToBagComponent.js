...
import { BUYNOW_ADDTOBAG_COMPONENT, BUYNOW, ADDTOBAG, WISHLIST } from "../../ComponentConstants";
import {
    PRODUCT_CART_ROUTER,
...
export default class BuyNowAddToBagComponent extends React.Component {
    ...
    checkIfSizeSelected = () => {
        ...
        if (this.props.productDetails.variantTheme && this.props.productDetails.variantTheme.length > 0) {
            const variantOptions = this.props && this.props.productDetails && this.props.productDetails.variantTheme;
            sizeOptions = variantOptions && variantOptions.length > 0 && variantOptions.map(el => el.sizelink);
            const productListingId =
                this.props && this.props.productDetails && this.props.productDetails.productListingId;
            selectedSize =
                sizeOptions &&
                sizeOptions.length > 0 &&
                sizeOptions.filter(el => {
                    if (el) {
                        return (
                            el.productCode === productListingId && el.isAvailable === true && el && el.selected === true
                        );
                    }
                });
            if (sizeOptions && sizeOptions.length === 0) {
                colorOptions = variantOptions && variantOptions.length > 0 && variantOptions[0].colorOptions;
                // eslint-disable-next-line no-unused-vars
                selectedColor =
                    colorOptions &&
                    colorOptions.length > 0 &&
                    colorOptions[0].filter(el => {
                        if (el) {
                            return (
                                el.productCode === productListingId && el.isAvailable === true && el.selected === true
                            );
                        }
                    });
                return true;
    ...
    render() {
        ...
        const compDetails = this.props && this.props.compDetails ? this.props.compDetails : [];
        const addToBagBuyNowComp = compDetails.filter(el => el.componentId === BUYNOW_ADDTOBAG_COMPONENT);
        const buttonToShow =
            addToBagBuyNowComp &&
        ...
        disabledStatus =
            ...
            (productDetails.winningSellerAvailableStock === "0" && this.checkIfSizeSelected());
        if (productDetails.isServiceableToPincode) {
            if (
                "productNotServiceableMessage" in productDetails.isServiceableToPincode ||
            ...
            <React.Fragment>
                {!this.state.showGotoCartButton && (
                    <div className={styles.buttonWrapper}>
                        {buttonToShow && buttonToShow.filter(el => el.key === BUYNOW).length > 0 && (
                        ...
                        {buttonToShow && buttonToShow.filter(el => el.key === ADDTOBAG).length > 0 && (
                        ...
                        {buttonToShow && buttonToShow.filter(el => el.key === WISHLIST).length > 0 && (
