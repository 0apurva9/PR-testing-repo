    ...
    render() {
        ...
        const pincodeListResponse =
            ...
            this.props.productDetails.pincodeResponseList.deliveryOptions.pincodeListResponse
                ? this.props.productDetails.pincodeResponseList.deliveryOptions.pincodeListResponse
                : [];
        stockCount = pincodeListResponse.filter(el => el.ussid === winningUssID).map(el => el.stockCount);
        ...
        if (this.state.sizeAndColorComp.length === 2) {
            ...
            selectedSizeSelectedColor =
                selectedSizeColorOptions && selectedSizeColorOptions.filter(el => el.selected === true);
        } else {
            selectedColorOptions =
                Array.isArray(variantTheme) && variantTheme.length > 0 && variantTheme[0].colorOptions;
                ...
                <div className={styles["shade-component"]}>
                    <div className={styles["shade-block"]}>
                        <div className={styles["shade-top-block"]}>
                            {(noOfColoursForSelectedSize && noOfColoursForSelectedSize > 7) ||
                            (variantTheme && variantTheme.length > 1) ||
                            (noOfColours && noOfColours > 7) ? (
                                <a
                                    href={""}
                            ...
                            <div className={styles["shade-heading"]}>
                                {this.state.sizeAndColorComp.length === 2
                                    ...
                                    : selectedColor && selectedColor.length > 0 && selectedColor[0].color}
                            </div>
                        </div>
                        <div
                            className={
                        ...
                        >
                            {variantTheme &&
                                ...
                                variantTheme.map((colorAndSize, i) => {
                                    if (
                                        this.state.sizeAndColorComp &&
                                        this.state.sizeAndColorComp.length === 1 &&
                                        i > 0
                                    ) {
                                        return null;
                                    }
                                    if (colorAndSize.colorOptions && colorAndSize.colorOptions.length > 0) {
                                        return (
                                            <div key={i}>
                                                {this.state.sizeAndColorComp &&
                                                this.state.sizeAndColorComp.length === 2 ? (
                                                    <div className={styles["shade-subheading"]}>
                                                        {colorAndSize.sizelink && colorAndSize.sizelink.size
...
ColorComponent.propTypes = {
    compDetails: PropTypes.any,
    handleScrollToTop: PropTypes.func,
    history: PropTypes.shape({
        location: PropTypes.shape({
            viewMoreLess: PropTypes.shape({
                viewMoreLess: PropTypes.any,
            }),
        }),
        push: PropTypes.func,
    }),
    productDetails: PropTypes.shape({
        pincodeListResponse: PropTypes.shape({
            deliveryOptions: PropTypes.shape({
                pincodeListResponse: PropTypes.arrayOf(
                    PropTypes.shape({
        ...
        pincodeResponseList: PropTypes.shape({
            deliveryOptions: PropTypes.shape({
                pincodeListResponse: PropTypes.any,
            }),
        }),
        productListingId: PropTypes.string,
        variantTheme: PropTypes.arrayOf(
            PropTypes.shape({
