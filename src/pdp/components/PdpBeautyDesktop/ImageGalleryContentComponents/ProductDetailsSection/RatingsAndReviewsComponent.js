...
export default class RatingsAndReviewsComponent extends React.Component {
    ...
    render() {
        const { averageRating, ratingCount, numberOfReviews, displayRatingReview } =
            this.props && this.props.productDetails;
        let averageRatingNew = null;
        if (averageRating) {
            averageRatingNew = Math.round(averageRating * ADJUST_FACTOR) / ADJUST_FACTOR;
        }

        return (
            <React.Fragment>
                {averageRatingNew && displayRatingReview && (
                    <div
                    ...
                    >
                        <div className={styles["review-rating-block"]}>
                            ...
                            {averageRatingNew > 2.5 && (
                                <span className={[styles["rating-star"], styles["rating-star-green"]].join(" ")} />
                            )}
                            {averageRatingNew <= 2.5 && (
                                <span className={[styles["rating-star"], styles["rating-star-orange"]].join(" ")} />
                            )}
