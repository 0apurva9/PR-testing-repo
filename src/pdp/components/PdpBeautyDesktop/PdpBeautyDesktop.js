...
export default class PdpBeautyDesktop extends React.Component {
        ...
        if (sectionToScroll && sectionToScroll === "ratingsLong") {
            if (this.ratingReviewsRef.current) {
                ...
                window.scrollTo({
                    behavior: "smooth",
                    top:
                        document.getElementById("rating-parent").getBoundingClientRect().top -
        ...
        if (sectionToScroll && sectionToScroll === "detailsLong") {
            if (this.detailsRef.current) {
                ...
                window.scrollTo({
                    behavior: "smooth",
                    top:
                        document.getElementById("details-parent").getBoundingClientRect().top -
                        document.body.getBoundingClientRect().top -
                        offset,
                });
            }
    ...
    componentDidMount = () => {
        ...
        let productId = this.props.productDetails ? this.props.productDetails.productListingId : null;
        if (productId) {
        ...
        if (categoryHierarchy.length > 0) {
            masterCategoryId = categoryHierarchy[categoryHierarchy.length - 1].category_id;
        }
        if (masterCategoryId) {
    ...
    render() {
        ...
        sortedMasterTempLateDetails =
            ...
            sortArrayOfObjectByIntegerKeyValue(masterTemplateDetails, "componentPosition");
        ...
        if (sortedMasterTempLateDetails && sortedMasterTempLateDetails.length > 0) {
            ...
            const ingredientDetails = productDetails.ingredientDetails ? productDetails.ingredientDetails : [];
            ...
            const imageGalleryTemplateData =
                ...
                sectionOfImageAndContentComponent.filter(el => el.componentId === IMAGE_GALLERY_COMPONENT);
...
PdpBeautyDesktop.propTypes = {
    ...
    getHowToWear: PropTypes.func,
    getMoreFromBrand: PropTypes.func,
    getSimilarProduct: PropTypes.func,
    getAboutTheBrand: PropTypes.func,
    getMasterTemplate: PropTypes.func,
