...
export default class ProductGalleryDesktop extends React.Component {
    ...
    swapViews(prev, next) {
        if (prev > -1 && next > -1 && next < this.props.productImages.length && prev !== next) {
            this.setState({
                previousPage: prev,
                nextPage: next,
                position: next,
    ...
    render() {
        let nonYoutubeImageUrl = this.props.productImages.find(el => el.type === "image").value;
        return (
            <div className={styles.base}>
                <div className={styles.content}>
                    {this.props.productImages.map((val, i) => {
                        ...
                        if (this.state.position === i || this.state.previousPage === i || this.state.nextPage === i) {
                            ...
                            if (val.type === "video") {
                                let videoUrl = val.value.split("/");
                                const isYoutube = videoUrl.some(
                                    el => el.includes("youtu.be") || el.includes("youtube")
                                );
                                if (!isYoutube) {
                            ...
                            return (
                                <div className={imageClass} id="APIM">
                                    {val.type === "image" && (
                                        <Image image={val.value} fit="contain" alt={this.props.alt} />
                                    )}
                                    {val.type === "video" && (
                                        <Video
                                            url={val.value}
                                            controls={true}
                                            light={!isYoutubeVideo ? nonYoutubeImageUrl : null}
                                            playIcon={!isYoutubeVideo ? <div className={styles.play} /> : null}
                                            playing={!isYoutubeVideo ? true : false}
                                        />
                                    )}
                                    <div className={styles.similarIcon} onClick={() => this.showSimilarProducts()}>
                                        <Icon image={similarIcon} size={38} backgroundSize="auto 22px" />
                                    </div>
                                </div>
                            );
                        } else {
                    ...
                    {this.type === "image" && (
                        ...
                        >
                            <div
                                className={this.state.isZoom ? styles.zoomArea : styles.hideZoomArea}
                                style={{
                    ...
                    {this.props.thumbNailImages.map((val, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {this.props.productImages[i].type === "video" && <div className={styles.videoImage} />}
                            </div>
                        );
                    })}
                        ...
                        {this.props.category === "Clothing" && (
                            <React.Fragment>
                                <div className={styles.marginTop10}>
                                    <div className={styles.description}>{this.getKeyValue(MODEL_FIT)}</div>
                                </div>
                                <div className={styles.marginTop10}>
                                    <div className={styles.fabtype}>
                                        {this.getKeyValue(FABRIC)}
                                        {this.getKeyValue(WASH) !== undefined && this.getKeyValue(WASH) !== null && (
                                            <span>, </span>
                                        )}
                ...
                <div className={this.state.isZoom ? styles.zoomVisible : styles.zoom}>
                    ...
                    >
                        <Image image={this.props.zoomImages[this.state.position]} fit="contain" alt={this.props.alt} />
                    </div>
                </div>
            </div>
        );
