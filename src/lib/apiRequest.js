...
if (
    ...
    MIDDLEWARE_API_URL_ROOT = "https://uat2.tataunistore.com/marketplacewebservices";
} else if (process.env.REACT_APP_STAGE === "tmpprod") {
    API_URL_ROOT = "https://tmppprd.tataunistore.com/marketplacewebservices";
    MIDDLEWARE_API_URL_ROOT = "https://tmppprd.tataunistore.com/marketplacewebservices";
    ...
    MIDDLEWARE_API_URL_ROOT = "https://uat.tataunistore.com/marketplacewebservices";
} else if (process.env.REACT_APP_STAGE === "uat6tcs") {
    API_URL_ROOT = "https://uat6-tcs.tataunistore.com/marketplacewebservices";
    MIDDLEWARE_API_URL_ROOT = "https://uat6-tcs.tataunistore.com/marketplacewebservices";
...
async function corePost(path, postData, doNotUseApiSuffix = false, headers = {}) {
    const apiUrlRootObj = new URL(API_URL_ROOT);
    const url = `${doNotUseApiSuffix ? apiUrlRootObj.origin : apiUrlRootObj.href}/${path}`;
...
export async function get(url) {
    ...
    try {
        ...
        if (
            (!errorStatus.status || !isInvalidAccessTokenError(errorStatus.message)) &&
...
export async function post(path, postData, doNotUseApiSuffix: true, headers = {}) {
    const result = await corePost(path, postData, doNotUseApiSuffix, headers);
    const resultClone = result.clone();
    const resultJson = await result.json();
    const errorStatus = ErrorHandling.getFailureResponse(resultJson);
