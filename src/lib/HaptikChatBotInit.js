...
export async function HaptikChatBotInitSetup() {
  ...
  loadScripts(
    `window.haptikInitSettings = ${haptikInitSetup}`,
    customSignUpConfig
  );
}
...
async function getCustomSignUpConfig() {
  let customSignUpDataObj = {
    username:
      (userDetailsCookie && `${JSON.parse(userDetailsCookie).firstName} ${JSON.parse(userDetailsCookie).lastName}`) ||
      "Guest User",
    auth_id:
      (userDetailsCookie && JSON.parse(userDetailsCookie).customerId) ||
      generateRandomCharacterString(12),
  ...
  let scriptContent = `
    document.addEventListener('haptik_sdk', function() {
      HaptikSDK.signup(${JSON.stringify(
    customSignUpDataObj
