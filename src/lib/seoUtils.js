...
export const getPdpSchemaMetaTags = () => {
    return (
        <MetaTags>
    ...
    if (productDetails.type === "productSearchPageWsDto") {
        ...
        if (
            ...
            description = `Shop for top ${productDetails.searchresult[0].brandname} ${
                ...
                productDetails.seo.title.split("|")[0]
            } by ${productDetails.searchresult[0].brandname} with Free Shipping✯COD✯Easy Returns.`;
            keywords = `${productDetails.searchresult[0].brandname} ${productDetails.seo.title.split("|")[0]}, ${
                productDetails.searchresult[0].brandname
            } ${productDetails.seo.title.split("|")[0]} Online, Buy ${productDetails.searchresult[0].brandname} ${
                productDetails.seo.title.split("|")[0]
            }, Buy ${productDetails.searchresult[0].brandname} ${productDetails.seo.title.split("|")[0]} Online`;
        } else {
            if (productDetails.seo !== undefined && productDetails.seo.title !== undefined) {
                title = `Buy ${productDetails.seo.title.split(" ")[0]} - Upto ${maxDiscount}% Off Online - TATA CLIQ`;
    ...
    return (
        <MetaTags>
            ...
            {window.location.href.indexOf("viewSellers") === -1 ? (
                <link rel="canonical" href={`${URL_ROOT}${canonicalUrl}`} hrefLang="en-in" />
            ) : null}

            <link rel="alternate" href={`${URL_ROOT}${alternateUrl}`} hrefLang="en-in" />
...
export const renderMetaTagsWithoutSeoObject = () => {
    ...
    return (
        <MetaTags>
            ...
            <link rel="canonical" href={isBrowser ? window.location.href : ""} hrefLang="en-in" />
            <link rel="alternate" href={isBrowser ? window.location.href : ""} hrefLang="en-in" />
            {renderOgTags()}
        </MetaTags>
    );
};
...
export const renderOgTags = (productDetails, isReviewPage = false) => {
    ...
    return (
        <React.Fragment>
            ...
            {googleDescription && <meta itemProp="description" content={googleDescription} />}
            ...
            {facebookDescription && <meta property="og:description" content={facebookDescription} />}
