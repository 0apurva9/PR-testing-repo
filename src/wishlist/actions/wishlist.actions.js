import {
  ...
  import { showSecondaryLoader, hideSecondaryLoader } from "../../general/secondaryLoader.actions.js";
  import {
  ...
  export const REMOVE_PRODUCT_FROM_WISH_LIST_REQUEST = "REMOVE_PRODUCT_FROM_WISH_LIST_REQUEST";
  export const REMOVE_PRODUCT_FROM_WISH_LIST_SUCCESS = "REMOVE_PRODUCT_FROM_WISH_LIST_SUCCESS";
  export const REMOVE_PRODUCT_FROM_WISH_LIST_FAILURE = "REMOVE_PRODUCT_FROM_WISH_LIST_FAILURE";
  ...
  export function addProductToWishList(productDetails, setDataLayerType) {
	  const userDetails = Cookie.getCookie(LOGGED_IN_USER_DETAILS);
	  const customerCookie = Cookie.getCookie(CUSTOMER_ACCESS_TOKEN);
	  return async (dispatch, getState, { api }) => {
		  ...
		  try {
			  ...
			  if (setDataLayerType === SET_DATA_LAYER_FOR_SAVE_PRODUCT_EVENT_ON_PDP) {
				  setDataLayerForPdpDirectCalls(SET_DATA_LAYER_FOR_SAVE_PRODUCT_EVENT_ON_PDP);
  ...
  export function removeProductFromWishList(productDetails, onPLP = null) {
	  return async (dispatch, getState, { api }) => {
		  ...
		  if (productDetails.removeFromWlWithExchange) {
			  productToBeRemove.append("removeFromWlWithExchange", productDetails.removeFromWlWithExchange);
		  ...
		  try {
			  const result = await api.postFormData(
				  `${PRODUCT_DETAILS_PATH}/${JSON.parse(userDetails).userName}/removeProductFromWishlist?access_token=${
					  JSON.parse(customerCookie).access_token
				  }&isMDE=true`,
				  productToBeRemove
			  );
	  ...
	  return async (dispatch, getState, { api }) => {
		  ...
		  try {
			  const result = await api.postFormData(
				  `${PRODUCT_DETAILS_PATH}/${
					  JSON.parse(userDetails).userName
				  }/CreateWishlist?channel=${CHANNEL}&access_token=${JSON.parse(customerCookie).access_token}&isPwa=true`,
				  createWishlistObj
			  );
			  const resultJson = await result.json();
			  const resultJsonStatus = ErrorHandling.getFailureResponse(resultJson);
